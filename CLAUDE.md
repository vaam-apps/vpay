...
- `sdks/tauri/tauri-plugin-vpay-checkout/` is **its own Cargo workspace**
  (ADR-0023, T2), so `cargo nextest run --workspace`, `just clippy`,
  `cargo deny` and `verify-no-mocks` never see it — a change there can break
  the crate with `just ci` still green. Use `just test-tauri-rust`,
  `just clippy-tauri-rust` and `just check-tauri-mobile`; the TypeScript half
  needs nothing, `pnpm -r` already reaches it. Two more things that will cost
  you an hour: a bare `cargo check --target aarch64-apple-ios` used to exit
  101 because `swift-rs` compiles the plugin's Swift for **iOS 13.0** by
  default while the Swift needs 15.0 — an `if #available(iOS 15.0, *)` guard
  in `VpayCheckoutExternalBrowserSession.swift` is what makes it pass without
  `IPHONEOS_DEPLOYMENT_TARGET=15.0`, so a Swift availability error there is
  that guard regressing, not your environment. And `permissions/autogenerated/`
  and `permissions/schemas/` are **tracked and regenerated by every
