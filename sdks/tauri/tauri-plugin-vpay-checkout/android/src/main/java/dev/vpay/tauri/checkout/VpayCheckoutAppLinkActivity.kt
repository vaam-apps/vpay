// The plugin's ONE exported entry point, ported from
// `sdks/flutter/vpay_checkout_flutter/android/src/main/kotlin/dev/vpay/checkout_flutter/VpayCheckoutAppLinkActivity.kt`
// with its reasoning intact. Every other component in this package is
...
//
// SECURITY TRADEOFF, stated loudly rather than left implicit: this Activity
// IS directly addressable by any other app on the device, via an explicit
...
//
// Why that is an accepted cost and not a hole: D1 means the guest-JS
// checkout state machine NEVER trusts a `stopUrlReached` event, or its
// `reachedUrl`, as proof of anything — it always re-polls the real
// `GET /v1/browser/payment_intents/{id}` before deciding
// succeeded/canceled/failed, and `resolveAfterStopUrlReached` does not even
// take the URL as an argument. The worst a spoofed event can do is close
// the sheet early and trigger that poll, which still resolves against the
// true payment state. This class also never sees the checkout session's own
...
//
// Tauri changes nothing about the above. A Tauri v2 Android app is one
// `TauriActivity` in one task, and a plugin module's manifest is merged
// into that app's manifest by AGP, so this exported Activity ends up in the
// merchant's app exactly as it does in a Flutter merchant's — with the same
// addressability and the same mitigation.
//
...
// in their own manifest (for a Tauri app,
// `src-tauri/gen/android/app/src/main/AndroidManifest.xml`).
// `AndroidManifest.xml`'s comment here carries the snippet and the reason:
// the only filter a shared plugin could write is
...
// scheme+host+port+path match (D2) — and always finishes itself immediately
