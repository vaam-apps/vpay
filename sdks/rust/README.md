...
out of the graph, `reqwest`'s own builder panics unless the *application* has
...
4. **Re-authenticate on expiry or a `401`.** There is no refresh token, by
   ...
   `401` is returned to you. A failure from the *token endpoint* is never
...
| `verify_assertion` | Feeds an assertion to the real `authkestra_op` verifier. Backs `just sdk-conformance-node`, which checks the *Node* SDK's assertions against it. |
...
The Rust and Node SDKs must put the *same bytes* on the wire for the same
...
- Token caching (a second call makes no second token request), expiry
  ...
  single `401`-triggered re-auth — proven by matching on the *new* bearer
...
- Webhook verification: a valid signature, the wrong secret, a timestamp
  outside tolerance, a timestamp exactly *on* the tolerance boundary
