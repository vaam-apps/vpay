...
#     migrations are immutable (`verify-migrations`)
#
...
# Regenerate the migration manifest from the current state of migration files.
#
# The manifest records the SHA256 hash of each migration file at the time this
# recipe is run. It refuses to modify existing entries (though new lines may
# be appended), so that `verify-migrations` cannot be satisfied by
# regenerating after an accidental edit.
#
# A migration file is immutable once applied to any database. If a file must
# be fixed, create a new migration instead — see docs/runbooks/migrations.md.
migrations-manifest:
    #!/usr/bin/env bash
    set -eu
    ...
    # Build a temporary manifest from the current files
    ...
    find "$migrations_dir" -maxdepth 1 -type f -name "*.sql" | sort | while read -r file; do
        ...
        echo "$sha256  $filename" >> "$temp_manifest"
    done

    # If the manifest exists, check that no existing line has changed
    if [ -f "$manifest" ]; then
        while IFS='  ' read -r existing_hash existing_filename; do
            # Extract the line for this filename from the temporary manifest
            temp_line=$(grep "^.*  $existing_filename$" "$temp_manifest" || true)
            if [ -n "$temp_line" ]; then
                temp_hash=$(echo "$temp_line" | awk '{print $1}')
                if [ "$existing_hash" != "$temp_hash" ]; then
                    echo "migrations-manifest: REFUSED — $existing_filename has been modified." >&2
                    echo "Applied migrations are immutable. Do not edit migration files." >&2
                    echo "To fix a bug, create a new migration instead." >&2
                    echo "See docs/runbooks/migrations.md for details." >&2
                    exit 1
                fi
            fi
        done < "$manifest"
    ...
    # Safe to write: either the manifest doesn't exist, or all existing lines
    # match their current hashes
    mv "$temp_manifest" "$manifest"
    echo "migrations-manifest: ok — regenerated $manifest"
