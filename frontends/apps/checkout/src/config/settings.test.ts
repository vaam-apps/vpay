...
`);
  ...
  it("counts characters, not UTF-16 units, so an emoji is one", () => {
    ...
    expect(
      parseBranding(`display_name: "${"\u{1f41d}".repeat(80)}"\n`).value
        .displayName,
    ).not.toBeNull();
    expect(
      parseBranding(`display_name: "${"\u{1f41d}".repeat(81)}"\n`).value
        .displayName,
    ).toBeNull();
  });
