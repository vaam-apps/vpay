...
import { DEFAULT_BRANDING_PATH, DEFAULT_CONFIG_PATH, readRuntimeConfig } from './runtime';

const made: string[] = [];

function mount(files: Record<string, string>): { brandingFile: string; configFile: string } {
  const dir = mkdtempSync(join(tmpdir(), 'vpay-checkout-config-'));
  made.push(dir);
  for (const [name, body] of Object.entries(files)) {
    writeFileSync(join(dir, name), body, 'utf8');
  }
  return { brandingFile: join(dir, 'branding.yaml'), configFile: join(dir, 'config.yaml') };
}
...
describe('readRuntimeConfig', () => {
  it('reads both mounted files', () => {
    const paths = mount({
      'branding.yaml': 'display_name: Vaam Payments\nprimary_color: "#f3c623"\n',
  ...
  it('tells a broken mount apart from an empty one', () => {
    const paths = mount({ 'branding.yaml': 'display_name: X\n', 'config.yaml': 'checkout: {}\n' });
  ...
  it('reads one file when only one is mounted', () => {
    const paths = mount({ 'config.yaml': 'checkout:\n  features:\n    page_memory: false\n' });
    const { config, problems } = readRuntimeConfig(paths);
  ...
  it('takes its paths from the environment at RUNTIME, not from a build-time literal', () => {
    ...
    try {
      ...
      expect(readRuntimeConfig().config.branding.displayName).toBe('From the env');
    } finally {
