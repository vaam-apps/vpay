...
function renderState(
  state: CheckoutState,
  locale: Locale,
  overrides: Partial<CheckoutViewProps> = {},
) {
...
describe("every screen renders in every locale", () => {
  it("covers each state the machine can be in", () => {
    expect(Object.keys(CHECKOUT_SCREENS).sort()).toEqual(
      Object.keys(EXPECTED_SCREEN).sort(),
    );
  });
  ...
  for (const [name, state] of Object.entries(CHECKOUT_SCREENS)) {
    for (const locale of LOCALES) {
      it(`renders ${name} in ${locale}`, () => {
        ...
        expect(heading?.getAttribute("data-screen")).toBe(
          EXPECTED_SCREEN[name],
        );
...
describe("the summary", () => {
  it("shows the amount in minor units correctly — 5000 XAF is five thousand", () => {
    ...
    expect(
      screen.getByTestId("amount").textContent?.replace(/[^0-9.]/g, ""),
    ).toBe("5000");
    unmount();
  });
  ...
  it("shows the test-mode banner while livemode is false, and hides it otherwise", () => {
    ...
    const live = structuredClone(state) as CheckoutState & {
      context: { session: { livemode: boolean } };
    };
...
describe("a session whose read carried no merchant name", () => {
  ...
  const UNNAMED: Record<string, CheckoutState> = {
    ...
    expired: {
      name: "expired",
      context: makeContext({ status: "expired" }, {}, null),
    },
    outcome: {
      name: "outcome",
      context: makeContext(
  ...
  for (const locale of LOCALES) {
    it(`shows the neutral heading rather than a hole in a sentence (${locale})`, () => {
      const { container, unmount } = renderState(
        UNNAMED["collect_msisdn"] as CheckoutState,
        locale,
      );
      expect(text(container, "pay-to")).toBe(
        DICTIONARIES[locale]["page.pay_to_unnamed"],
      );
      unmount();
    });
  }
  ...
  it("puts no identifier, and no stand-in that reads like data, where the name would be", () => {
    const { container, unmount } = renderState(
      UNNAMED["outcome"] as CheckoutState,
      "en",
    );
  ...
  it("says the neutral sentence on every screen the name appears in", () => {
    ...
    expect(text(expired.container, "notice-body")).toBe(
      DICTIONARIES.en["expired.body_unnamed"],
    );
    expired.unmount();
    ...
    expect(
      outcome.container.querySelector("[data-outcome] button")?.textContent,
    ).toBe(DICTIONARIES.en["outcome.back_to_unnamed"]);
    outcome.unmount();
  });
  ...
  it("still shows the name when the read carried one, in both sentences", () => {
    ...
    expect(
      container.querySelector("[data-outcome] button")?.textContent,
    ).toContain("Boutique Test");
    unmount();
  });
  ...
  it("does the same on the return page", () => {
    ...
    expect(text(container, "pay-to")).toBe(
      DICTIONARIES.fr["page.pay_to_unnamed"],
    );
...
describe("accessibility", () => {
  it("moves focus to the new screen’s heading", () => {
    ...
    expect(document.activeElement).toBe(
      container.querySelector("[data-screen]"),
    );
    unmount();
  });
  ...
  it("gives every control an accessible name and a keyboard-reachable element", () => {
    for (const [name, state] of Object.entries(CHECKOUT_SCREENS)) {
      ...
      for (const control of controls) {
        ...
        const native = ["BUTTON", "INPUT", "SELECT", "A"].includes(
          control.tagName,
        );
        if (!native) {
          expect(
          ...
          if (control.getAttribute("role") === "checkbox") {
            expect(["true", "false"], `${name}`).toContain(
      ...
      for (const impostor of container.querySelectorAll(
  ...
  it("ties the MSISDN hint and error to the field, whatever ids Base UI mints", () => {
    ...
    expect(screen.getByTestId("msisdn-problem").getAttribute("role")).toBe(
      "alert",
    );
    unmount();
  });
...
describe("the controls do what the screen says", () => {
  ...
  it("reports the rail a payer picked, by code", () => {
    ...
    fireEvent.click(
      container.querySelector('[data-rail="orange_money"]') as HTMLElement,
    );
  ...
  it("offers one named button back to the merchant, and no timer", () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "Back to Boutique Test" }),
    );
  ...
  it("keeps the brand-and-language row a banner landmark on both views", () => {
    ...
    const checkout = renderState(
      CHECKOUT_SCREENS["collect_msisdn"] as CheckoutState,
      "en",
    );
    expect(within(checkout.container).getByRole("banner").tagName).toBe(
      "HEADER",
    );
    checkout.unmount();
...
describe("the return view", () => {
  for (const [name, state] of Object.entries(RETURN_SCREENS)) {
    for (const locale of LOCALES) {
      it(`renders ${name} in ${locale}`, () => {
        ...
        expect(
          container.querySelector("[data-screen]")?.textContent?.trim(),
        ).not.toBe("");
        unmount();
      });
    }
  ...
  it("shows the failure the intent reported, in the payer’s language", () => {
    ...
    expect(container.textContent).toContain(
      DICTIONARIES.fr["failure.payer_timeout"],
    );
    unmount();
  });
});
...
describe("branding, from the deployment’s own branding.yaml", () => {
  ...
  it("shows the operator’s name, logo and support contact when they are", () => {
    ...
    expect(screen.getByTestId("support-contact").textContent).toContain(
      "support@vaam.example",
    );
    unmount();
  });
  ...
  it("never lets the operator’s name stand in for the merchant’s", () => {
    ...
    expect(screen.getByTestId("pay-to").textContent).toBe(
      DICTIONARIES.en["page.pay_to_unnamed"],
    );
    expect(screen.getByTestId("pay-to").textContent).not.toContain(
      "Vaam Payments",
    );
    ...
    const elsewhere = [...container.querySelectorAll("*")].filter(
      (node) =>
        node !== brand && !brand.contains(node) && !node.contains(brand),
    );
...
describe("the rail’s own words on a failure", () => {
  it("shows them under the translated sentence, never instead of it", () => {
    ...
    expect(detail.textContent).toContain(
      DICTIONARIES.en["outcome.provider_said"],
    );
...
describe("page memory, on the entry screens", () => {
  ...
  it("says so once the device has been cleared, rather than leaving the payer guessing", () => {
    ...
    expect(screen.getByTestId("forgotten").textContent).toBe(
      DICTIONARIES.en["memory.forgotten"],
    );
    unmount();
  });
  ...
  it("offers the same opt-in on the redirect screen, naming the rail rather than a number", () => {
    const { unmount } = renderState(
      CHECKOUT_SCREENS["ready_redirect"] as CheckoutState,
      "en",
    );
    const box = screen.getByRole("checkbox");
  ...
  it("marks the rail this device last used without choosing it", () => {
    ...
    expect(marked.closest("button")?.getAttribute("data-rail")).toBe(
      "orange_money",
    );
