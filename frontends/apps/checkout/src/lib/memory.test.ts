...
describe("parseMemoryRecord", () => {
  it("reads a record this page wrote", () => {
    expect(
      parseMemoryRecord(
        { msisdn: GOOD, rail: "mtn_momo", savedAt: NOW - 1000 },
        NOW,
      ),
    ).toEqual({
      msisdn: GOOD,
  ...
  it("drops a number that is not a canonical Cameroon MSISDN", () => {
    for (const msisdn of [
      ...
      const record = parseMemoryRecord(
        { msisdn, rail: "mtn_momo", savedAt: NOW },
        NOW,
      );
  ...
  it("keeps the good half of a half-bad record", () => {
    expect(
      parseMemoryRecord(
        { msisdn: "nonsense", rail: "mtn_momo", savedAt: NOW },
        NOW,
      ),
    ).toEqual({
      msisdn: null,
  ...
  it("treats a record that remembers nothing as no record", () => {
    expect(
      parseMemoryRecord({ msisdn: null, rail: null, savedAt: NOW }, NOW),
    ).toBeNull();
  ...
  it("forgets a record older than the ninety-day horizon", () => {
    ...
    expect(
      parseMemoryRecord({ msisdn: GOOD, rail: null, savedAt: justInside }, NOW),
    ).not.toBeNull();
    expect(
      parseMemoryRecord(
        { msisdn: GOOD, rail: null, savedAt: justInside - 1 },
        NOW,
      ),
    ).toBeNull();
  });
  ...
  it("refuses a record from the future — a clock moved, and the age is meaningless", () => {
    expect(
      parseMemoryRecord({ msisdn: GOOD, rail: null, savedAt: NOW + 1 }, NOW),
    ).toBeNull();
  });
  ...
  it("refuses a savedAt that is not a finite number", () => {
    for (const savedAt of [
      ...
      expect(
        parseMemoryRecord({ msisdn: GOOD, rail: null, savedAt }, NOW),
      ).toBeNull();
    }
  });
...
describe("memoryRecordFor", () => {
  ...
  it("builds a rail-only record for a redirect rail, which collects no number here", () => {
    expect(
      memoryRecordFor({ msisdn: null, rail: "orange_money" }, NOW),
    ).toEqual({
      msisdn: null,
      rail: "orange_money",
      savedAt: NOW,
  ...
  it("refuses a number the page would not have sent", () => {
    ...
    expect(
      memoryRecordFor({ msisdn: "+237 671 234 567", rail: null }, NOW),
    ).toBeNull();
  });
});
...
describe("pageMemoryFor", () => {
  ...
  it("offers the store when the deployment allows it and the browser has one", () => {
    expect(pageMemoryFor(true, store)).toEqual({
      memory: store,
      offered: true,
    });
  });
  ...
  it("offers nothing, and forgets, when config.yaml turned the feature off", () => {
    ...
    expect(pageMemoryFor(false, store)).toEqual({
      memory: NO_PAGE_MEMORY,
      offered: false,
    });
  });
  ...
  it("offers nothing when the browser has no IndexedDB", () => {
    ...
    expect(pageMemoryFor(true, null)).toEqual({
      memory: NO_PAGE_MEMORY,
      offered: false,
    });
    expect(pageMemoryFor(false, null)).toEqual({
      memory: NO_PAGE_MEMORY,
      offered: false,
    });
  });
