...
describe("frameAncestors", () => {
  ...
  it("lists the origins, space-separated, in order", () => {
    expect(
      contentSecurityPolicy(["https://a.example", "https://b.example"]),
    ).toBe("frame-ancestors https://a.example https://b.example");
  });
...
describe("the origins request header", () => {
  ...
  it("re-validates on the way in, so a forged header cannot smuggle a wildcard", () => {
    expect(decodeOriginsHeader("* https://shop.example")).toEqual([
      "https://shop.example",
    ]);
  });
});
