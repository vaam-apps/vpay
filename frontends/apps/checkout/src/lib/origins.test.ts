...
describe("originOf", () => {
  it("returns the origin of an absolute http(s) URL", () => {
    expect(originOf("https://shop.example/pay?x=1#y")).toBe(
      "https://shop.example",
    );
...
describe("resolveParentOrigin", () => {
  ...
  it("refuses a framer that is not on the list", () => {
    expect(
      resolveParentOrigin("https://evil.example/checkout", allowed),
    ).toBeNull();
  });
  ...
  it("refuses a look-alike origin", () => {
    expect(
      resolveParentOrigin("https://shop.example.evil.test/", allowed),
    ).toBeNull();
    expect(resolveParentOrigin("http://shop.example/", allowed)).toBeNull();
    expect(
      resolveParentOrigin("https://shop.example:8443/", allowed),
    ).toBeNull();
  });
});
