  ...
  it("carries every key in both locales, with a non-empty value", () => {
    for (const locale of LOCALES) {
      ...
      for (const key of KEYS) {
        expect(dictionary[key], `${locale} is missing ${key}`).toBeTypeOf(
          "string",
        );
  ...
  it("translates every key without leaving a placeholder unfilled", () => {
    ...
    for (const locale of LOCALES) {
      ...
      for (const key of KEYS) {
        ...
        expect(rendered, `${locale}.${key} kept a placeholder`).not.toMatch(
          /\{[a-z_]+\}/,
        );
...
describe("format", () => {
  it("substitutes named placeholders", () => {
    expect(
      format("Pay {merchant} {amount}", { merchant: "Shop", amount: "5" }),
    ).toBe("Pay Shop 5");
  });
