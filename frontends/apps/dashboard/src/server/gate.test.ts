...
describe("the gate", () => {
  ...
  it("treats an empty token as no token", () => {
    expect(gateFor({ ...SESSION, access_token: "" }, at(0)).kind).toBe(
      "needs-token",
    );
  });
...
describe("replacing the token before it expires (issue #88 item 1)", () => {
  ...
  it("reads with the token it holds for the first four fifths of its life", () => {
    ...
    expect(gateFor(SESSION, at(TTL_SECONDS - marginSeconds - 1)).kind).toBe(
      "ready",
    );
  });
  ...
  it("re-mints when vpay sent no expiry at all", () => {
    ...
    expect(
      gateFor({ ...SESSION, access_token_expires_at: "not a date" }, at(0))
        .kind,
    ).toBe("stale-token");
    expect(
      gateFor({ ...SESSION, access_token_ttl_seconds: 0 }, at(0)).kind,
    ).toBe("stale-token");
  });
...
describe("what /login/totp must do about the session it read", () => {
  ...
  it("renders the code form for a session that has not presented one yet", () => {
    expect(totpGateFor({ stage: "pending_totp" }, null).kind).toBe(
      "enter-code",
    );
  });
  ...
  it("sends a session that already has both factors on to the payments list", () => {
    ...
    // replay.
    expect(totpGateFor({ stage: "authenticated" }, null).kind).toBe(
      "signed-in",
    );
  });
