...
describe("the origin check in front of every server action", () => {
  ...
  it("refuses another origin however the request was addressed", () => {
    expect(
      originIsAllowed("https://evil.example", PUBLIC, "dash.example"),
    ).toBe(false);
  });
  ...
  it("cannot be satisfied by a forwarded host, because it never sees one", () => {
    ...
    expect(
      originIsAllowed("https://evil.example", PUBLIC, "evil.example"),
    ).toBe(false);
  });
  ...
  it("treats a different port as a different origin", () => {
    expect(
      originIsAllowed("https://dash.example:8443", PUBLIC, "dash.example"),
    ).toBe(false);
  });

  it("treats a different scheme as a different origin", () => {
    expect(originIsAllowed("http://dash.example", PUBLIC, "dash.example")).toBe(
      false,
    );
  });
  ...
  describe("with no configured public origin", () => {
    it("falls back to the Host header — and only to Host", () => {
      expect(
        originIsAllowed("http://localhost:3000", null, "localhost:3000"),
      ).toBe(true);
      expect(
        originIsAllowed("http://localhost:3000", null, "localhost:3001"),
      ).toBe(false);
    });
    ...
    it("is not weakened by a blank configured value", () => {
      ...
      expect(
        originIsAllowed("https://evil.example", "   ", "dash.example"),
      ).toBe(false);
    });

    it("does not treat an unparseable configured value as permission", () => {
      expect(
        originIsAllowed("https://evil.example", "dash.example", "evil.example"),
      ).toBe(true);
      expect(
        originIsAllowed("https://evil.example", "dash.example", "dash.example"),
      ).toBe(false);
    });
  });
});
...
describe("origin normalisation", () => {
  it("keeps scheme, host and port and drops everything else", () => {
    ...
    expect(normaliseOrigin("http://dash.example:3000/")).toBe(
