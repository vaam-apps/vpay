...
function refusal(status: number, message: string): Response {
  return new Response(
    JSON.stringify({ error: { code: 'invalid_token', message, type: 'authentication_error' } }),
    { status, headers: { 'content-type': 'application/json', 'request-id': 'req_01J8' } },
  );
}
...
function page(): Response {
  return new Response(
    JSON.stringify({ object: 'list', data: [{ id: 'pi_one' }], has_more: false }),
    { status: 200, headers: { 'content-type': 'application/json' } },
  );
}
...
describe('reading /dash/v1', () => {
  it('uses the token from the session row and exchanges nothing when it works', async () => {
    const seen: Seen[] = [];
    vi.stubGlobal('fetch', stub(seen, () => page()));

    const result = await readDash(CONFIG, '/dash/v1/payment_intents', EXPIRED, SESSION);
  ...
  it('mints a fresh token once when the one it holds has expired — THE decisive case', async () => {
    ...
    vi.stubGlobal(
      ...
      stub(seen, (bearer) =>
        bearer === `Bearer ${FRESH}`
          ? page()
          : refusal(401, 'The bearer token is invalid, expired, or was not issued for this endpoint.'),
      ),
    );
    ...
    expect(seen.some((call) => call.url.includes('/oauth/authorize'))).toBe(true);
  ...
  it('gives up after one mint rather than looping on a credential operation', async () => {
    ...
    const seen: Seen[] = [];
    vi.stubGlobal('fetch', stub(seen, () => refusal(401, 'still refused')));
  ...
  it('never retries a 403 — a new token from the same registration says the same thing', async () => {
    ...
    const seen: Seen[] = [];
    vi.stubGlobal('fetch', stub(seen, () => refusal(403, 'not allowed here')));
  ...
  it('reports the read refusal, not the exchange, when no token can be minted', async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn((input: string | URL | Request, init?: RequestInit) => {
        ...
        return Promise.resolve(refusal(401, 'the read refusal a page should render'));
      }),
    );
