...
import { decodePendingEnrolment, encodePendingEnrolment, secretFrom } from './enrolment';

const PENDING = {
  sealed: 'c2VhbGVkLWJsb2I',
  otpauth: 'otpauth://totp/vpay%20sandbox:ada%40example.test?secret=GEZDGNBVGY3TQOJQ&issuer=vpay%20sandbox',
};

describe('the pending enrolment cookie', () => {
  it('round-trips both halves', () => {
    expect(decodePendingEnrolment(encodePendingEnrolment(PENDING))).toEqual(PENDING);
  });
  ...
  it.each<[string, string | undefined]>([
    ...
    ['missing the sealed half', encodeURIComponent(JSON.stringify({ otpauth: PENDING.otpauth }))],
    ['missing the URI', encodeURIComponent(JSON.stringify({ sealed: PENDING.sealed }))],
    [
      'a URI that is not otpauth',
      encodeURIComponent(JSON.stringify({ sealed: 'x', otpauth: 'https://example.test/?secret=x' })),
    ],
