...
describe('Input', () => {
  ...
  it('tone="ghost" is the borderless variant, tone="error" the invalid one', () => {
    ...
    expect(screen.getByRole('textbox', { name: 'A' }).className).toContain('input-ghost');
    unmount();

    const second = render(<Input tone="error" aria-label="B" />);
    expect(screen.getByRole('textbox', { name: 'B' }).className).toContain('input-error');
    second.unmount();
  });

  it('a className override wins over the variant, via cn', () => {
    const { unmount } = render(<Input tone="ghost" className="input-error" aria-label="C" />);
