 ...
 */
function themeColours(root: postcss.Root): Map<string, [number, number, number]> {
...
const compiled = await postcss([tailwind()]).process(readFileSync(STYLES, 'utf8'), {
  from: STYLES,
});
...
describe('the shipped theme, measured rather than trusted', () => {
  it('emits a colour and a foreground for every tone this product renders', () => {
    for (const tone of TONES) {
      ...
      expect(colours.get(`${tone}-content`), `--color-${tone}-content`).toBeDefined();
    }
  });
  ...
  for (const tone of TONES) {
    it(`clears WCAG AA (4.5:1) for --color-${tone}-content on --color-${tone}`, () => {
      ...
      const ratio = contrastRatio(fill as [number, number, number], ink as [number, number, number]);
  ...
  it('does not let a component start using a tone nobody has measured', () => {
    ...
    const sources = readdirSync(dir)
      ...
      .join('\n');
    expect(sources.length, 'the component sources were found').toBeGreaterThan(1000);
    for (const tone of UNRENDERED_TONES) {
      expect(sources, `a component now offers "${tone}" — measure it above`).not.toMatch(
        new RegExp(`\\b(alert|badge|btn|checkbox|radio|select|input)-${tone}\\b`),
      );
    }
  });
  ...
  it('keeps the override that makes error and info readable, rather than daisyUI 5 bumblebee’s own', () => {
    ...
    expect(error, `error ${error.toFixed(2)}:1 — daisyUI 5 bumblebee's own is 3.53`).toBeGreaterThan(
      4.5,
    );
    expect(info, `info ${info.toFixed(2)}:1 — daisyUI 5 bumblebee's own is 4.27`).toBeGreaterThan(
      4.5,
    );
  });
});
