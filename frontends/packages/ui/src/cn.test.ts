...
describe("cn", () => {
  ...
  it("daisy-btn-style: a btn colour and a btn style COMPOSE, they do not conflict", () => {
    ...
    expect(cn("btn btn-primary", "btn-outline")).toBe(
      "btn btn-primary btn-outline",
    );
    expect(cn("btn btn-outline", "btn-primary")).toBe(
  ...
  it("daisy-badge-variant: a later badge colour replaces an earlier one", () => {
    expect(cn("badge badge-neutral", "badge-success")).toBe(
      "badge badge-success",
    );
  });
  ...
  it("daisy-badge-style: a badge tone and a badge style COMPOSE", () => {
    expect(cn("badge badge-success", "badge-outline")).toBe(
  ...
  it("daisy-select-variant: a later select modifier replaces an earlier one", () => {
    expect(cn("select select-ghost", "select-error")).toBe(
      "select select-error",
    );
  });
  ...
  it("daisy-checkbox-variant: a later checkbox colour replaces an earlier one", () => {
    expect(cn("checkbox checkbox-primary", "checkbox-error")).toBe(
      "checkbox checkbox-error",
    );
  });

  it("daisy-checkbox-size: a later checkbox size replaces an earlier one", () => {
    expect(cn("checkbox checkbox-lg", "checkbox-sm")).toBe(
  ...
  it("daisy-loading-type: a later loading type replaces an earlier one", () => {
    expect(cn("loading loading-spinner", "loading-dots")).toBe(
  ...
  it("falsy and conditional inputs are dropped, like clsx alone", () => {
    ...
    const count = 0;
    expect(
      cn("btn", disabled && "btn-ghost", null, undefined, count && "x"),
    ).toBe("btn");
  });
});
