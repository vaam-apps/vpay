...
**And it is a trap, written down 2026-09-23 because it has already bitten
once.** The lock records `tauri-plugin-vpay-checkout` **by version** —
...
— even though the dependency in `src-tauri/Cargo.toml` is by **path**. So a
hand-edit of the crate's `version` leaves this file stale, and **no gate in
this repository catches it**:

- `--locked` appears here only on `cargo install` lines (`justfile:92-93`,
  and the `cratestack-cli` message at `justfile:1920`);
  `.github/workflows/release-please.yml`'s own comment says as much.
- `cargo xtask verify-versions` reads manifests and
  `release-please-config.json`. It never opens a lockfile.
- `src-tauri/Cargo.toml` carries its **own `[workspace]`** (see its header),
  so no root cargo command — `cargo nextest run --workspace`, `just clippy`,
  `cargo deny`, `verify-no-mocks`'s `cargo metadata` — ever resolves it.

vpay #240 did exactly this: it moved the plugin to `0.4.1` and left this
lock at `0.4.0`. #241 had to repair it, and the maintainer found it by
building on real hardware.

**The gap is narrower than it sounds, and the narrow part is worth
knowing.** The **release** path self-heals:
`.github/workflows/release-please.yml`'s "Refresh Cargo.lock" step walks
`git ls-files '*Cargo.lock'` — which is now **two** files, this one and the
root's — and runs `cargo metadata --format-version 1` in each directory, so
a release-please bump updates this lock on the release branch. The exposure
is the **manual** bump only. If you edit
`sdks/tauri/tauri-plugin-vpay-checkout/Cargo.toml`'s `version` by hand, do
this in the same change:

```bash
cd examples/tauri-checkout/src-tauri
cargo metadata --format-version 1 > /dev/null   # rewrites Cargo.lock
git add Cargo.lock
```

## What `just ci` does and does not cover here

Written down 2026-09-23, because it had been written down nowhere and the
plugin crate's equivalent is written down five times. This is the more
important of the two: **this example is the only thing in the repository
that can compile the plugin's Kotlin and its Swift.**

- **Covered.** `pnpm-workspace.yaml` globs `examples/*`, so this package is
  an ordinary pnpm workspace member. `just ci` runs `lint-web`
  (`pnpm -r typecheck`, `pnpm -r lint`) and `test-web` (`pnpm -r test`), so
  this package's `typecheck` and `lint` really do run in CI — and each of
  them chains `pnpm run deps` first, which builds
  `@vaam-apps/vpay-tauri-checkout`, so the guest-JS is built in CI too.
  `fmt-check-web`'s `prettier --check .` reaches every file here.
- **Not covered, and it is exactly two things.** `src-tauri/` — the Rust,
  and with it every route to the plugin's `android/**/*.kt` and
  `ios/**/*.swift` — plus any script `pnpm -r` never calls, which today is
  **`dev`**. Both halves of #241 sat in that slice: a stale
  `src-tauri/Cargo.lock`, and a `dev` script that did not build its own
  dependency.
- **Why.** `src-tauri/Cargo.toml` carries its own `[workspace]` table, so
  ADR-0023's T2 isolation applies a second time and no root cargo command
  sees this crate. Measured on `dd1a48b`: `examples/tauri-checkout` appears
  in **0** justfile recipes, **0** files under `.github/workflows/` and
  **0** places in `.xtask/src/`.

No gate is proposed here. Adding one (`cargo check --locked` in
`src-tauri/`) would move the machine-checked `count:verify-gates` figure,
and the justfile is explicit that putting Tauri Rust into CI is a
runner-image change and the maintainer's call.
...
| `cargo build` in `src-tauri` — **only after the row above**                    | exit 0, and the clause added 2026-09-23 is load-bearing rather than tidy: this row is exit 0 **because** `pnpm build` ran immediately before it. `tauri.conf.json` sets `frontendDist: "../dist"`, `.gitignore` ignores `/dist`, and `tauri-codegen` reads that directory at compile time, so on a clean checkout a bare `cargo build` here fails before it compiles anything. `beforeBuildCommand: "pnpm build"` does not save you — the Tauri CLI runs it, `cargo` does not. Run `pnpm --filter @vpay/example-tauri-checkout build` first, or use `pnpm exec tauri build`. |
