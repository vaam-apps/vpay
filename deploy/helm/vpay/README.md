...
| `NetworkPolicy`           | `<release>-management`                                                   | Optional (also needs `management.enabled`); admits only `networkPolicy.managementIngress.podSelector`, deliberately NOT the ingress controller namespace                |
...
- All **22** guards fire on their own values file, each with its own name in
  the message, and `just helm-check` also checks that the twenty-three names it
  expects are exactly the twenty-three files on disk — so deleting a guard _and_
  ...
  twenty-three. Measured:
  `23 guards, all fired by name (23 expected)`.) Proven negatively too, which is the
