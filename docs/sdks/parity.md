...
| The invoice's **nineteenth** key, `amount_refunded` (migration `0042`, issue #91 D5) — decoded, and **gross**: not subtracted from `amount_paid`, so a refunded invoice is still `paid` with nothing remaining. Tolerant of a server that predates `0042` (`#[serde(default)]` / optional), and the tolerance is exercised rather than merely declared. **Corrected 2026-09-11 (exp47 review):** this row said the *live* case was what proved the server sends the key. It is not, on the Rust side — `#[serde(default)]` decodes an absent key to `0` and the live assertion is `== 0`, so it passes either way; renaming the field's wire key left all 165 cases green. The offline case below now decodes a **non-zero** value and a body with the key removed. The Node live case does observe presence, because a missing key reads `undefined` there | ✅ `an_invoices_amount_refunded_is_read_from_the_wire_and_defaults_only_when_absent`, `create_invoice_sends_the_documented_body_and_decodes_every_key`, `live_invoice_lifecycle` | ✅ `invoices.create: exact path, method, Idempotency-Key, and body`, `creates a draft, bills two lines, finalizes, reads back and voids` |
