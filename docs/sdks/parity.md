...
| The invoice's **nineteenth** key, `amount_refunded` (migration `0042`, issue #91 D5) — decoded, and **gross**: not subtracted from `amount_paid`, so a refunded invoice is still `paid` with nothing remaining. Tolerant of a server that predates `0042` (`#[serde(default)]` / optional), which is why the *live* case is the one that proves the server sends it at all | ✅ `create_invoice_sends_the_documented_body_and_decodes_every_key`, `live_invoice_lifecycle` | ✅ `invoices.create: exact path, method, Idempotency-Key, and body`, `creates a draft, bills two lines, finalizes, reads back and voids` |
