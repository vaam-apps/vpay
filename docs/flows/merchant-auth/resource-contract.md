...
**Nor does it list the invoice routes, for the same reason.** Fifteen of
`V1_ROUTES`' 37 method-and-path pairs are invoices' and invoice items':
`POST`/`GET /v1/invoices`; `GET`, `POST`, `PATCH` and `DELETE
/v1/invoices/{id}`; `POST /v1/invoices/{id}/finalize`, `/void`,
`/mark_uncollectible` and `/pay`; `POST /v1/invoice_items`; and `GET`,
`POST`, `PATCH` and `DELETE /v1/invoice_items/{id}`. Their contract is
[invoices.md](../invoices.md) § "The surface". _(Added 2026-09-23. This
document was silent on them from the day they were mounted, 2026-09-07
(S4b), and the `DELETE /v1/customers/{id}` row below called that route "the
only `DELETE` on this API" — written 2026-09-06 and true for one day, until
the invoice routes made it one of three.)_
...
| `GET`    | `/v1/payment_intents`              | `limit`, `starting_after`, `ending_before`, `customer`                                               | `list` of `payment_intent`          | ✅; `customer` since 2026-09-23 (RFC-0004 § 5)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `GET`    | `/v1/refunds`                      | `limit`, `starting_after`, `ending_before`, `payment_intent`, `customer`                             | list of `refund`                    | ✅ since 2026-09-16; `customer` since 2026-09-23 (RFC-0004 § 5), through the refund's intent                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
...
| `DELETE` | `/v1/customers/{id}`               |                                                                                                      | `{"id", "object", "deleted": true}` | ✅ The erasure. ~~The only `DELETE` on this API~~ One of three `DELETE`s since 2026-09-07 — `DELETE /v1/invoices/{id}` and `DELETE /v1/invoice_items/{id}` are the others (corrected 2026-09-23, see above) — so one of the routes where the `Idempotency-Key` this table requires on every write is carried on a verb with no body. Since 2026-09-10 it never answers a `409`: a customer nothing references is hard-deleted, one an intent, a session or an invoice references is anonymised in place (migration `0041`, [customers.md](../customers.md)) |
