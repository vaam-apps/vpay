...
| `charge.refunded`               | `vpay_api::v1::refunds::create`, in the same transaction as the `refunds` row and its reservation                                                                                                                                                        | **2026-09-16** (RFC-0003 § 2)                                                                   |
| `charge.refund.updated`         | `vpay_api::v1::refunds`' `update_once`, `cancel_once` and `fail_with_event`, each in the transaction of the write it reports                                                                                                                             | **2026-09-16** (RFC-0003 § 2)                                                                   |
...
| `invoice.paid`                  | `vpay_db::settlement::apply_succeeded` (TX 1), and since **2026-09-23** `vpay_api::v1::invoices::write_with_event` for `POST /v1/invoices/{id}/pay` with `paid_out_of_band=true` (RFC-0004 § 6) — tell them apart by the body's `paid_out_of_band`       | 2026-09-07                                                                                      |
