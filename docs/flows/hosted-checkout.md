...
| `Content-Security-Policy` on that page    | `frame-ancestors 'none'`                                        | `frame-ancestors` followed by the merchant's `checkout_origins`                                 |
...
erasure now reaches those charges through the session. For the same reason,
a session create now locks its customer before the intent. See
