...
| [mobile-checkout.md](mobile-checkout.md)             | `vpay_checkout_flutter`: the same hosted page in a native window, the poll-not-URL outcome, dismissal handling, and the App Store / Play constraints on adoption (ADR-0021)                              |
| [tauri-checkout.md](tauri-checkout.md)               | `tauri-plugin-vpay-checkout`: the same payer surface on Tauri v2 — one guest-JS state machine, an Android/iOS/desktop host and a plain-browser popup, and what each of the four cannot signal (ADR-0023) |
...
| [invoices.md](invoices.md)                           | The `Invoice`: draft → open → paid \| void \| uncollectible as compare-and-swaps, per-merchant numbers, paying through the hosted checkout or out of band, refunds, events                               |
