...
|---|---|
| `staff_sessions.access_token_expires_at` (migration `0040`) | The app has to decide *before* the expiry and had no way to know when that was |
...
something to fall back on. A vpay that cannot be *reached* for the re-mint
...
first of its two assertions, *"the same expiry back means nothing re-minted"*,
