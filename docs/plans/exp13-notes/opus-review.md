...
postgres/mod.rs`, "Known gaps": _"Multi-column and zero-column CHECK
...
one of its columns."_ `constraints.rs` carries that filter verbatim.
...
| 4   | `--strict` removed from the invocation                                                                                                                                                       | must not write into the repository | **red** — on the exit-code assertion (`Some(0)` vs `Some(1)`), before the no-writes assertions are reached. It is still loud, and the repository is still untouched (`--out-dir` is outside the checkout). But the run _did_ write a snapshot into the out-dir, and pre-fix that directory leaked; see finding 1.     |
...
1. **[robustness → fixed `7a0255d`] The out-dir leaked on every failing run.**
   ...
   in a sibling binary. `tempfile` was correctly _not_ added: it is in neither
...
5. **[misleading-claim → fixed `d16eca2`] "No baseline row was recorded" was
   ...
   `--strict` is _asserted_ not to do; it had been checked by hand. It is the
