...
_do_ carry `#[test]`, _are_ collected by nextest, and _did_ run in the draft's
...
_discussed_ in comments — a naive count reads `model DisabledClient`'s long
...
are `vpay-db` _module_ and _trait_ names (`config_reconcile.rs`,
...
prose warns about — 0.11.1 derives a table name from the _model_ name, which
