...
worktree from here), but nothing under `frontends/apps/dashboard` in _this_
...
- **Nothing, as it turns out — `shop-embedded.cy.ts` completed on the
  ...
  under measurably _worse_ contention (load average peaking at 82.97
...
- **Mutation named in plan §5 Lane D's own table, in full — this is the one
  ...
  different shape: a _theme name_ that silently stops styling anything is
  the same failure mode as a _class name_ that does.
