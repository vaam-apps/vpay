...
*claim about what runs* is in [`../../status.md`](../../status.md) and in the
...
* **The unconditional rung** turns the 405 ms margin into ~10 s (the first
  rung of the ladder). It is armed by the *submit*, not by the page, so it
...
* **The click arming** is what makes a payer's action matter. It needed one
  ...
  point *straight* at the merchant's URL, so a payer who used one never
...
So the entire cost is: *an Orange charge answered by the catch-all settles one
rung (≈10 s) later than it did.* In the Rust suites that is
...
race lost still produces. In the *cancel* case of the same run the click
landed at T + 0.11 s, 170 ms *ahead* of the first poll, so it won the race and
...
* *is there a window at all, and is it bounded* → `backends/tests/conformance`,
  where there is no worker and the five polls are five function calls;
* *is the window long enough for a human* → `the_pending_chain_gives_a_payer_
  at_least_thirty_seconds`, arithmetic over the mapping file and
  `vpay_worker::poll_delay`;
* *does the payer's click decide the charge* → the Orange cancel case in
...
* **Not made concurrent-safe.** One scenario per container means two Orange
  ...
  next submit, so nothing *inherits* a state, but overlap is not fixable in
