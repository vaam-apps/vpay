...
- **No secret in the event body or in a log.** The unit case and the
  integration case both assert on the serialised _string_; mutation 3 proves
...
- The second parity row is phrased as a statement about what a _delivered_
...
- **Pre-existing, not introduced, and out of scope:** a payer who confirms
  inside the _single-statement_ window of `expire_due`'s `UPDATE` — after its
