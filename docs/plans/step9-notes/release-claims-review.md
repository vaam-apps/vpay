...
- dated history (b): `status.md:657` and `:998` (both under a
  ...
  _before this pass_"), `status.md:1592`, the nine `docs/plans/*` lines,
...
than assumed: nowhere does it claim the packages are _reachable_. Every
...
| M2  | `33929374661` → `39999999999` throughout `runbooks/release.md`                                  | `just verify`, `just docs-check`, `actionlint`, the grep, the link check | ~~**not caught by anything** — all five green.~~ **Caught since 2026-09-05** by `just docs-check-citations` (`cargo xtask verify-citations`), which resolves every cited run, PR and issue id against GitHub. `gh run view 39999999999` → `HTTP 404`. Finding F3: the proof, not the docs.                                                                                  |
| M3  | Break `../adr/0014-builder-host-musl-triple.md` (a link the change touched)                     | link check                                                               | **caught** by the reviewer's checker (`MISS` ×2, exit 1). ~~**Not caught** by `just docs-check`, which prints `note: link checking is not implemented yet` and exits 0 — confirming the notes' own disclosure.~~ **Caught by `just docs-check` since 2026-09-05:** that recipe now runs `cargo xtask verify-links`, and the "not implemented" echo is gone.                 |
...
It is the _test-evidence_ family that was left incomplete, and that is F1/F2 —
