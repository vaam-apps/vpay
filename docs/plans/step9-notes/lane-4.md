...
- **The checkout app got a `/healthz` route, which is the one file this lane
  ...
  on none: `middleware.ts`'s origins lookup already fails _closed_, so
...
- **The chart templates the checkout page and still does not template the
  ...
  `USER node`, and it has been _run_ — non-root, `--read-only --tmpfs /tmp`,
...
and still put `mtn_momo` on EUR, because MTN's _real sandbox_ rejects XAF
...
_absence_: "enabled: false renders nothing" is checked by grepping the default
...
- **No browser has rendered vpay's checkout page.** Step 5 mints two sessions
  and stops; `curl` on the hosted URL proves the page is _served_, not that it
