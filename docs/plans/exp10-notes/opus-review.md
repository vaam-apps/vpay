...
| no enum was given the attribute                     | all 13 additions in the diff are `struct`; `apply_to_variant` is _not_ the identity, so this is load-bearing                                                                                                                                              | true            |
...
1. **From serde's source.** `serde_derive-1.0.229`'s
   ...
   for `SnakeCase` — the identity, _unconditionally_, for any input. This is
...
all and never touches a wire — it is populated _from_ `sub` in a `From` impl.
...
the store is still _wired into_ the token handler).
...
its set from two signals that each find a _declaration_; neither finds
...
requires its _declaration_ to be `pub(crate)`. That is half of the standard's
...
**The `pub`-only scope question.** The maintainer's rule 3 says "every _public_
...
- Whether each exemption's _reason_ is true of the rail's live documentation.
