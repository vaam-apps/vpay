...
_contradicts_ the no-persistence rule the same section asks for. That is a
...
there is nothing to scope, and binding it is what keeps the _authentication_
...
| M13 | leak _one half_ of the holder's name                           | —                                                     | CAUGHT (the widened assertion)                                                       |
...
for the _joined_ `"Amina Nkeng"`: a form the adapter produces and the rail
...
1. **Rate limiting, audit logging, `identity:read`** (issue §3). The flow
   ...
   condition — _not safe to expose to untrusted merchants without ingress
   rate limiting_ — rather than only as a bullet in a list of absences. A
...
4. **`404 -> Ok(None)`.** Weighed and **kept**. Treating an undocumented 404
   ...
   compounds with the unverified path-segment case: the two wrong _together_
