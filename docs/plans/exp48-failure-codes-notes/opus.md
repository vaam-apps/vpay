...
| _(unpublished)_ `COULD_NOT_PERFORM_TRANSACTION` | `payer_timeout`                     | mapped, §1.3                   |
| _(unpublished)_ `SENDER_ACCOUNT_NOT_ACTIVE`     | `payer_account_blocked`             | mapped, §1.3                   |
...
producer, and the honest answer is that the gaps are _per rail_:
...
first mutation decisive at the _reason_ level rather than only at the code
