...
none is in scope (issue #11's own "Out of scope"). So the bar is: every gap that does _not_ need a real
rail is closed, proven by a test that fails when it breaks, and demonstrable from nothing with `just`
and Docker Compose. Everything that _does_ need a real rail stays a named ⛔/🟡 row in `docs/status.md`,
...
| C   | `docs/flows/adapter-mtn-momo.md`: "**No callback route exists.**" Both adapters implement `parse_callback`; nothing mounts a route, so `X-Callback-Url`/`notif_url` are never sent and settlement is polling-only | Both rails' documented protocol is push-then-callback. Polling alone works (Step 4 proves it) but a real MTN sandbox registration _requires_ a callback host, and the poll ladder's first hop is seconds, not instant |
...
_Written 2026-09-04 by lane E, on the merged gate branch `claude/step8-production-gate`
...
`1c742a4` after this section was written; its own counts name that commit._
...
| **D — a real `SIGKILL`**                       | `backends/tests/integration/tests/worker_kill9.rs`: the shipping `vpay-worker-bin` killed mid-status-query and the shipping `vpay-server` killed mid-`requesttopay`, exit asserted _signalled with 9_, the charge settling exactly once by four independent counts. Guard-failure proof: disabling `reap_expired_leases` leaves the second worker healthy and permanently unable to claim                                                                                                                                                                                                                                                                                                                                                                                  | Kill point 1 is still written rather than caused — there is no request to interrupt at that instant. Orange is not exercised. Two clocks are simulated (the dead worker's lease; since lane G, the crashed charge's age)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
4. **Lane H was not in the plan either.** It exists because Step 8's own
   ...
   the four sat in lane G's _fix_ — a guard whose age came off the worker
...
- **The `.e2e/` per-project keypair.** Compose-layer isolation is proven; the
  ...
  here is a _silent_ `invalid_client`.
