...
it: an undeclared column is absent from the generated model _struct_ as well
...
| `idle_since`, `delete_idle`            | raw `sqlx`                   | correlated `NOT EXISTS` over two _other_ tables |
...
asserts the compiled descriptor with no container, and asserts the _absence_
...
Not "harder": _different_. The cursor is `seq < (SELECT seq FROM customers
...
### `touch_last_used` is monotonic by _filter_, because `GREATEST` is not renderable
...
It matters because `now` is the _calling process's_ instant, two vpay
...
was a _second copy_ of the same list, in the test module. I updated that copy
...
**What I did _not_ claim.** There is no construction at 1.98.0 that makes a
...
rather than calling `delete_idle`, and it only caught it _loudly_ after I made
...
because it is an undeclared _table_ and the report collapses it to one line.
...
- `@@index([merchant_id])` cannot express `(merchant_id, seq DESC)`.
  ...
  line _beside_ the `[safe]` one rather than closing it — two lines to
...
asserts it directly and in _both_ directions (the refusal, and that a
...
| #   | Mutation                                                                                                                | Measured                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| --- | ----------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Delete `SweepIdleCustomers` from `JobKind::EVERY`                                                                       | `the_kinds_are_exactly_the_check_constraints` **FAILS in 4 ms**, no container, naming both lists. This is the _real_ bug of § 2, re-armed                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
`0023`'s own comment states the rule this repository holds itself to — _the
vocabulary moves in lockstep with the code that writes it, rather than being
written permissively ahead of it_ — and `0029` followed it exactly, adding one
type for one writer.

`payment_intent.created`, `.processing` and `.canceled` **are** in the list
with no writer, and they are the precedent for _not_ doing it again: they came
...
first cut … DECIDE by measuring"). I measured and it _is_ expressible — six
...
2. **`customer.created` / `customer.updated`.** § 6. The cost is turning
   ...
   row (to merge `metadata`), so it _can_ render the post-patch object — but
...
3. **The retention horizon is a constant, not configuration**, and I made that
   ...
   is a promise to a _payer_, and a deployment that could shorten it from a
...
4. **"Delete this customer" is not a complete erasure of the payer**, because
   ...
   worse, but a third answer — deleting the customer _and_ redacting the
