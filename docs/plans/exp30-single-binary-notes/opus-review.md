...
that one says what was built and why, this one says what was *checked*, what
...
| Signal handling | `ShutdownSignals::install()` first, before tracing | same call, now *before the subcommand dispatch* — stronger, not weaker | yes |
...
and `prometheusrule.yaml`'s five alerts key on metric *names*
...
binary refused them only *after* the subcommand word;
...
environment stays *ignored* rather than refused — which is what
...
sentence whose *meaning* changed: `vpay-api/src/staff_auth/mod.rs` kept the
...
with `command:` meaning the *entrypoint*). The kubelet half is still untested.
...
deleted as a *fact*. It is now a task with an owner and a named failure mode:
...
* **A SIGTERM that lands while a job is genuinely in flight** is exercised at
  ...
  the process level only with *nothing* in flight — `worker_kill9`'s
