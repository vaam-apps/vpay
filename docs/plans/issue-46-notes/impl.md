...
records what was built, what was _established_ rather than assumed, and what
...
from set that against a claim it attributed to a _pull request_ numbered 45.
...
_"SDK/API: a refund cannot be polled — `RefundsResource` has only `create()`,
and `GET /v1/refunds/{id}` is not in the wire contract"_. So the brief's
...
- Non-negative rather than positive, unlike `amount_positive` on the same
  table: a zero-amount refund is a caller mistake, a zero _fee_ is a real
...
_name_, so a different CHECK firing would not pass for it).
...
That is what lets `fee` exist on the refund path _without_ also appearing on
...
Parity row: _The `refund` object's `fee`, and the absent / `null` / `0`
distinction it exists for_, naming two tests per column.
...
| 4   | Delete `fee?:` from `sdks/nodejs`'s `Refund`            | `pnpm --filter @vaam-apps/vpay-sdk lint`, `… typecheck`, `… test`     | `lint` **passed**; `test` (vitest) **passed, 173/173**; `typecheck` **failed** with three `TS2339: Property 'fee' does not exist on type 'Refund'`. `just ci` catches it through `lint-web`, which runs `pnpm -r typecheck` — _not_ through `test-web`                                        |
...
- _"Remove the SDK field → the parity gate … FAIL."_ It does not.
  ...
  **exists** as a live `#[test]`/`it(…)`; it does not compile or run anything.
  ...
  and the _compiler_ is what fails. That is still a `just ci` failure, but it
...
- _"…and the SDK decode test FAIL."_ True in Rust (a compile error). In
...
- `verify`: _"ok — the ten gates above passed"_.
- `test-rust`: **1289 tests run, 1289 passed, 0 skipped** in 793 s, against
  real Postgres and WireMock containers (testcontainers).
- `verify-ignored`: _"0 ignored (expected 0), 41 test binaries (expected 41),
  1289 total"_. Base `65a5952` was 1279 across the same 41 binaries, so this
...
- `lint-web` (`pnpm -r typecheck` then `pnpm -r lint`): clean. `test-web`:
  ...
  unchanged.
- `deny`: _"advisories ok, bans ok, licenses ok, sources ok"_.

Separately, and **not** part of `just ci` because it needs the network:
`cargo xtask verify-citations` with a token — _"ok — 45 unique id(s) cited by
131 markdown file(s) all resolve"_. It failed on the first run; see section 1.
...
- `backends/crates/vpay-api/src/model.rs` — both branches added an
  ...
  _through_ the two macro calls rather than between them: the conflict shared
...
- `docs/flows/merchant-auth.md` — the **only** hunk where "keep both" would
  ...
  rewrote the _same_ "**Served** marks…" sentence. #49's edit was the date
...
- `fmt-check`, `clippy --workspace --all-targets -- -D warnings`: clean.
- `verify`: _"ok — the ten gates above passed; the verify-docs report is
  advisory"_.
- `cargo build --workspace --all-targets`: clean.
- `verify-ignored`: _"0 ignored (expected 0), 42 test binaries (expected 42),
  1330 total (minimum 1080)"_. `origin/master` at `1bd2183` was measured on
...
- **`test-rust` did NOT complete.** Two full `just ci` runs reached it and
  ...
  1019/1330 with _"container is not ready: container startup timeout"_, the
  second at 998/1330 with _"failed to create a container: Timeout error"_.
  ...
  `wiremock/wiremock:3.9.2` sit at _"Pulling fs layer"_ with **0 bytes**
...
Section 10 predicted this one: _"PR #51 adds a **second** refund renderer on
top of the same object. Whichever of #50 and #51 lands second will have to
rebase again, and the collision will be in the same place this one was."_ It
did, and it was.
...
4. **#51's integration suite moved 9 → 10 keys** and now asserts `fee` is
   _present and `null`_ for a seeded row that has none.
...
- **`backends/migrations/0031_refunds-fee.sql`** — its header and its
  ...
  true — nothing _writes_ it, so every stored value is `NULL`.
...
- `just verify`: _"ok — the ten gates above passed; the verify-docs report is
  advisory"_ — including `verify-sdk-parity: ok — 359 proving test(s) named in
...
- `just verify-ignored`: _"0 ignored (expected 0), 43 test binaries (expected
  43), 1342 total (minimum 1080)"_. No new test binary, so `expected_suites`
  stays 43.
...
_"672/1342 tests run: 671 passed, 1 failed"_, and the one failure is
`vpay-db::postgres an_abandoned_transaction_survives_a_rollback_it_cannot_send`
with _"failed to create a container: Error in the hyper legacy client (Connect)"_
...
1. `amount: row.amount` → `amount: row.amount - row.fee.unwrap_or(0)` —
   **`a_reported_fee_never_moves_the_payers_amount` FAILS**: _"a fee of
   Some(250) changed the amount the payer gets back — left: 1750, right:
   2000"_. Ten of the eleven filtered cases still pass, which is the point.
2. `fee: row.fee` → `fee: Some(row.fee.unwrap_or(0))` — **`an_unreported_
refund_fee_renders_null_and_a_reported_zero_renders_zero` FAILS**: _"left:
   0, right: Null"_. Three others fall with it
   (`the_refund_object_is_the_documented_ten_keys`,
   `the_merchant_sdk_deserialises_the_refund_this_renders`,
   `a_refund_delivered_as_either_refund_event_carries_fee_present_and_null`).
...
- `just verify`: ok — the ten gates. `verify-sdk-parity` now reports
  _"359 proving test(s) named in docs/sdks/parity.md all exist, 28 dated
  gap(s), **14 SDK method(s) enumerated across 17 row(s)**"_ — #52's second
...
- `just verify-ignored`: _"0 ignored (expected 0), 43 test binaries (expected
  43), **1359** total (minimum 1080)"_ — 1342 on the `bb8de92` base plus #52's
