...
What this file is for: the decisions that are *not* obvious from the code, the
...
The alternative is to treat a *declined* intent as immediately re-payable. It
...
consistent with the repository's standing rule that a retry is a *new*
...
be able to *choose* their prefix (Stripe lets a customer's be set). Nothing in
...
Migration 0032 had to *rename* `providers.flow`'s hand-named CHECK after the
...
its brackets *before* any decoding — deliberately, so a decoded `[` can never
