...
| GET         | `/v1/payment_intents`                  | `limit` (default 10, capped at 100), `starting_after`, `ending_before` (ids; not both), `customer` (a `cus_…`, since 2026-09-23, RFC-0004 § 5 — checked for shape only; another merchant's or an unknown one is an empty page, never a `404`; a malformed one is the `400` naming `customer` that `GET /v1/invoices` answers, byte for byte) — the intent's own `customer`                                                                                                                                                                                | `200` + `list` envelope (`list_pages_forward_and_backward_with_cursors`); `customer` pages inside the filtered set (`the_customer_filter_pages_inside_its_set_and_is_not_an_oracle`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| GET         | `/v1/refunds`                          | `limit` (default 10, capped at 100), `starting_after`, `ending_before` (`re_…` ids; not both), `payment_intent` (a `pi_…`; an id that is not yours is an empty page, never a `404`), `customer` (a `cus_…`, since 2026-09-23, RFC-0004 § 5 — checked for shape only; another merchant's or an unknown one is an empty page, never a `404`; a malformed one is the `400` naming `customer` that `GET /v1/invoices` answers, byte for byte) — matched through each refund's PaymentIntent, since a refund has no customer of its own                        | `200` + a list of `refund`, newest first (`the_customer_filter_goes_through_the_intent_and_is_not_an_oracle`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| GET         | `/v1/checkout/sessions`                | `limit`, `starting_after`, `ending_before`, `payment_intent`, `customer` (a `cus_…`, since 2026-09-23, RFC-0004 § 5 — checked for shape only; another merchant's or an unknown one is an empty page, never a `404`; a malformed one is the `400` naming `customer` that `GET /v1/invoices` answers, byte for byte) — the **session's own** `customer`, the one it renders, not its intent's                                                                                                                                                               | `200` + `list` envelope, **no secrets** (Step 9) (`the_customer_filter_reads_the_sessions_own_customer_and_is_not_an_oracle`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
