...
| POST        | `/v1/customers`                        | `name` (≤256), `email` (≤512), `phone` (a Cameroon mobile number in any of the three spellings above), `address[line1]`/`address[line2]`/`address[city]`/`address[state]` (≤256 each), `address[postal_code]` (≤64), `address[country]` (ISO 3166-1 alpha-2, upper-cased on the way in), `metadata[…]` — **at least one of `name`/`email`/`phone` is required, and `phone` alone is enough**; an address alone is not a customer                    | `201` + `customer`; `400` naming all three when none was sent; `400` naming `address` for a country that is not two letters, an over-long component, or a scalar `address=Douala` (S4a, #67)                                                                                                                                                                                                                          |
...
**`address` is one nested object of six nullable components** — `line1`,
`line2`, `city`, `state`, `postal_code`, `country` — or `null` when the
customer has none ([#67](https://github.com/vaam-apps/vpay/issues/67),
2026-09-10; this paragraph called it a gap until then). Send it as
`address[line1]=…` and so on; `country` is ISO 3166-1 alpha-2 and is
...
fact, not six, and a component-wise merge would assemble an address that was
