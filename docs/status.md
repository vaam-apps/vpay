...
because its new "boots" case asserted on the subprocess's *stderr* while
...
itself because this entry is *in* it — **issue #85: the graceful stop is a
...
constants untouched makes the case **fail in 20 seconds** with *"the worker
never logged `webhook delivered`"*, which reads as a regression in the drain
and is not one.
...
signal is seen) fails with *the worker never logged `webhook delivered`*: the
send was cut off after the receiver had already *received* it (WireMock
...
restarting: a worker spawned after a *clean* drain boots only once the drain
...
The restart therefore lives in the *timed-out* case, where there is
...
still earns its place: it proves the *function* over a whole `LoopReport`,
...
| Customers — `/v1/customers` (`vpay_api::v1::customers`, `vpay_db::customers`, migration `0034`) | ✅ | **New 2026-09-06 (S4a).** `cus_…` — the merchant-owned record of a payer, five routes (`create`/`retrieve`/`update`/`list`/`del`), both SDKs, and a twelve-month retention sweep that is **built and runs**, not `NotImplemented`. [flows/customers.md](flows/customers.md) is the whole document; the load-bearing facts: the maintainer's three decisions of 2026-09-05 are in the schema and in tests (`phone` stored by default with no opt-in; **a phone number alone is a complete customer** — `at_least_one_identifier` requires *one* of name/email/phone, proven in both directions against a real Postgres; retention is twelve months, `CUSTOMER_IDLE_AFTER`, deliberately not configurable because it is a promise to a payer). `phone` is canonicalised through the **same** `canonical_msisdn` the account-holder lookup uses, so vpay never holds two spellings of one payer. `DELETE` is a **hard** delete (no `deleted_at`, no `@@soft_delete` — pinned by a `preview_sql` test, because adding the attribute is a one-line schema edit that changes no Rust and would leave the API answering `{deleted: true}` for data it kept), and a customer any intent or session references **cannot** be deleted at all: the foreign keys are `NO ACTION`, turned into a `409` that says so. That is a stated trade — "delete this customer" is *not* a complete erasure of the payer, because the payment record survives; clearing the three identifiers with an update is the other half, which is why an update can clear a field and why clearing the *last* one is a `400` decided above the statement (the database's own CHECK is a `23514` → `Category::Storage` → a `503` telling a merchant to wait for a healthy database). `customer` on `POST /v1/payment_intents` **stopped being accepted-and-dropped** in the same migration and is stored and rendered; the intent object went 12 keys → 13. Proven by **18 container-backed cases** in `backends/tests/integration/tests/customers.rs` (the 44th test binary) driving the shipping router, the shipping SDK and the shipping worker loop, 3 in `postgres_smoke.rs` at the schema, 7 in `vpay-db` with no container, 2 in `vpay_api::model`, 8 in `sdks/rust` and 8 in `sdks/nodejs`. **✅ rather than 🟡, and the bar it is measured against:** every claim in this row fails a named test if it breaks, including the sweep's three cases (deleted / kept-because-recent / kept-because-referenced) and the confirm-path retention stamp. **Five of those cases were added by the sabotage review on 2026-09-07 and each closed a measured hole**, which is why the count moved after the row was first written: the object's key set was guarded by nothing (`the_customer_object_is_the_documented_eight_keys` — rendering the internal `last_used_at` on the wire was green across `vpay-api`, `vpay-sdk`, the integration suite and the Node SDK, and two documents named a tripwire of that name that did not exist; the count in them was also wrong, seven for eight); the list's tenancy and its **cursor** were guarded by nothing (`a_list_is_tenant_scoped_and_a_foreign_cursor_pages_nothing` — dropping the tenant filter from `list_page`'s cursor subquery passed all twelve delivered cases, and the forged cursor then pages another merchant's rows); and `POST /v1/customers`'s idempotency was guarded by nothing (`a_replayed_create_answers_the_stored_customer_and_a_reused_key_is_refused` — releasing the key instead of storing the response passed all twelve, and a retried create then mints a second `cus_…` for one payer). A fifth case, `a_sessions_customer_is_inherited_supplied_or_a_refused_contradiction`, covers the **checkout session** half, which S4a documented in four sentences and tested in none: inherited from the intent, supplied directly, refused when the two disagree, the same uniform `400` for another merchant's `cus_…`, and a customer that only a *session* references pinned against `DELETE` — deleting the contradiction check left the entire thirty-one-case `checkout_sessions.rs` suite green. **A gap that review also found and did not close: neither SDK models a session's `customer` in either direction** though the server accepts and renders it — dated ⛔/⛔ rows in [sdks/parity.md](sdks/parity.md), owned by the SDK maintainers. **Updated 2026-09-10 ([issue #66](https://github.com/vaam-apps/vpay/issues/66)): `POST /v1/customers` and `POST /v1/customers/{id}` are transactional, and each emits.** Migration `0039` adds `customer.created` and `customer.updated` to `type_is_a_documented_event` **in the same change that writes them** — migration `0023`'s lockstep rule working as intended, and the reason `0034` declined to add them. The create's event commits with the insert; the update's commits with the `UPDATE`, and the update's read is now `SELECT … FOR UPDATE`. **That lock is the part that mattered.** `metadata` is merged key-wise, so the written value is a function of the stored one, and `vpay_api::v1::customers::update`'s own doc comment said in as many words that two concurrent updates each adding one key could lose one — tolerable while nothing depended on the result being definite, and not tolerable once an event describes it. Three decisive mutations were run on 2026-09-10: removing `FOR UPDATE` fails `two_concurrent_metadata_merges_keep_both_keys_and_the_event_carries_the_committed_state` (six rounds, `{"right":"2"}` — the `left` key clobbered) **and** `a_locked_customer_read_waits_for_the_writer_and_then_sees_its_value`, which forces the interleaving deterministically at the seam rather than racing for it; dropping `customer.updated` from `0039` fails both the vocabulary case and the create/update case with a `23514`; renaming the Node parity test fails `verify-sdk-parity` naming that column. A **bodiless** update emits nothing, and a `404` writes nothing at all. Both SDK unions gained the two types, and the dated ⛔/⛔ parity row from 2026-09-06 is closed. What it does **not** claim, and the flow doc lists in full: no `address` on the object; no `email` filter on the list; neither SDK has run this resource against a live vpay; **no `customer.created` or `customer.updated` has been fanned out to a receiver in any suite** — the fan-out is type-agnostic and has been observed for other types, which is an argument and not a measurement; and **no deployment has ever run the retention sweep**, because no vpay has been up for twelve months |
| Invoices — `/v1/invoices` and `/v1/invoice_items` (`vpay_api::v1::invoices`, `vpay_api::v1::invoice_items`, `vpay_db::invoices`, migration `0036`) | ✅ | **New 2026-09-07 (S4b).** `in_…` and `ii_…` — a merchant's bill to one customer and the lines it is made of. Nine routes over five paths plus four over two, a per-merchant document number, four event types, and a payment through the checkout that already exists. [flows/invoices.md](flows/invoices.md) is the whole document; the load-bearing facts: **the state machine is the `WHERE` clause.** `draft → open → paid \| void \| uncollectible`, and every transition is `UPDATE … WHERE id = $1 AND merchant_id = $2 AND status = '<from>'`. There is no `can_transition_to` in this codebase — a Rust guard beside the write is the thing a future writer calls *instead of* taking the lock — and the API only ever reads an invoice **after** a refusal, to decide whether the answer is a `404` or a `409`. Five multi-column CHECKs (`number_is_assigned_at_finalize`, `paid_means_nothing_remaining`, `only_a_live_invoice_has_an_intent`, `amounts_add_up`, `amount_is_the_product`) make the combinations a broken transition would produce unstorable; all five are **invisible to `migrate baseline` in both directions**, exactly as `at_least_one_identifier` is, so `the_invoice_invariants_are_enforced_by_the_database_itself` writes the row each one refuses, straight past the API and the repository. **Numbers are a sequence and not a counter.** `{prefix}-{000001}`, per merchant, assigned at finalize under the row lock `INSERT … ON CONFLICT (merchant_id) DO UPDATE` takes — and `invoice_number_sequences` is an ordinary table row and **not** a Postgres `SEQUENCE`, because `nextval` is non-transactional and a rolled-back finalize would leave a hole a tax authority reads as a destroyed document. Stripe's numbering has holes; this one does not, and `two_concurrent_finalizes_take_consecutive_numbers` (two in-flight requests, distinct consecutive numbers, no gap) and `a_refused_finalize_does_not_burn_a_number` are the two halves. **The invoice is marked paid in TX1.** The settlement transaction — the one that takes the charge terminal — flips the invoice and emits `invoice.paid` inside itself; `an_aborted_settlement_leaves_the_invoice_open_and_emits_nothing` is the mutation proof (a replayed `event_id` aborts the transaction, the invoice is still `open`, no event describes a payment that did not commit), and `a_settlement_whose_invoice_moved_emits_no_invoice_event` proves the fail-closed direction. **One payment at a time**: `pay`, `void` and `mark_uncollectible` all refuse while an attached intent is not `canceled`, spelled once as `NO_LIVE_INTENT` and proven at the repository seam because the API's own read refuses the same cases and would mask the statement. Proven by **12 container-backed cases** in `backends/tests/integration/tests/invoices.rs` (the 45th test binary) driving the shipping router over a socket, **5** in `vpay-db`'s `tests/repositories.rs`, 3 in `vpay-db` with no container, and the drift/CHECK inventory in `postgres_smoke.rs`. **Five decisive mutations were run rather than argued**: drop the void transition guard → the terminal-transition case fails; drop the `ON CONFLICT` clause → the concurrency case fails; drop `NO_LIVE_INTENT` from `attach_intent` → the repository case fails; delete `@@allow("update")` on `model Invoice` → the no-container policy case fails; drop `invoice.paid` from the vocabulary → both the vocabulary case and the settlement case fail. What it does **not** claim, and the flow doc lists in full: **neither merchant SDK has one invoice method** and neither event union knows the four types (three dated ⛔/⛔ rows in [sdks/parity.md](sdks/parity.md) — the largest gap here, and the reason the suite drives raw HTTP); no Cypress case follows a `hosted_invoice_url`; no PDF, no e-mail, no hosted invoice page; no taxes, discounts or credit notes; no subscriptions or recurring invoices; no dunning and `due_date` is read by nothing; `invoice.marked_uncollectible` and `invoice.payment_failed` are not emitted because nothing writes them; `invoice.*` webhook bodies carry `lines.data` **empty**; no partial payments; and the dashboard has no invoice screen. **AMENDED 2026-09-07 by the S4b sabotage review**, which found three mutation escapes and two false claims and fixed all five. (1) **Two concurrent `POST /v1/invoices/{id}/pay` requests minted two intents and two hosted checkout URLs for one bill** with `NO_LIVE_INTENT` deleted from `attach_intent`, and every one of the twelve delivered wire cases stayed green — the delivered suite paid twice *in sequence*, so the handler's own read answered and the statement was never the thing under test. `two_concurrent_pays_attach_exactly_one_intent` puts both requests in flight over the socket and is what fails under that mutation now. (2) **Unscoping either list cursor left every case green** while another merchant's `in_…` as `starting_after` paged the caller's own rows — the existence oracle `dashboard_read_surface.rs` already pins for intents and nothing pinned here. `a_foreign_cursor_is_an_empty_page_and_never_an_oracle` straddles the foreign invoice with one older and one newer row of the caller's, because the obvious fixture ordering makes the mutation invisible and that was measured too. (3) **Keying the settlement's invoice flip on anything but its own intent left all seven delivered invoice/settlement cases green** while one intent's settlement paid an invoice it was never bound to; `a_settlement_pays_only_the_invoice_its_own_intent_is_bound_to` hands `apply_succeeded` a projection of the *wrong* invoice and asserts the statement, not the caller's body, chooses the row. (4) **An invoice could be finalized and paid past `2^53 - 1` minor units**: `pay` mints its intent through `PaymentIntents::insert` rather than `parse_amount`, so the bound `POST /v1/payment_intents` enforces — the point past which a JSON number stops round-tripping through a double, and the value both merchant SDKs check client-side — did not apply. Measured: ninety-one lines at both `invoice_items` parameter ceilings finalized `200` at 9,100,000,000,000,000. `finalize` now answers `400` naming `invoice`, at finalize rather than at `pay` (which would leave an `open` invoice nobody could pay) or at the line write (already committed by then). (5) **`docs/api/README.md` said the invoice object has seventeen keys; it has eighteen, and no test of any name held the number** — the same shape the customer object's review found the day before, inherited without its tripwire. Corrected, and `the_invoice_object_is_the_documented_eighteen_keys` now names the internal columns (`seq`, `merchant_id`, `updated_at`) that must never reach a `data.object` stored in `events` forever. Two documentation gaps were recorded rather than closed: **the payer's checkout page does not render the invoice number** (it is in the intent's `description` and on the wire the page reads — closing it is a change to `frontends/apps/checkout`, not to this resource), and **what a refund should do to a paid invoice is undecided and reserved for the maintainer** (unreachable today: nothing writes a `refunds` row). The suite is **12 → 15** container-backed cases and `vpay-db`'s is **5 → 6**. **Updated 2026-09-08 (exp33): the first of those three gaps is closed.** Both merchant SDKs now ship the whole resource — thirteen methods each and the four `invoice.*` event types — and the three dated ⛔/⛔ rows in [sdks/parity.md](sdks/parity.md) are fifteen ✅/✅ rows. ~~plus **one** ⛔/⛔ that stays: *invoices exercised against a running vpay*~~ **— that sixteenth row was closed the same day by the exp33 review**, which added a live suite to each SDK (`just sdk-live`, and CI's `e2e` job) and found, on the first run, that `currency` is required on `POST /v1/invoices` where both SDKs had documented it optional. `sdks/stripe-compat` still has **no invoice cases**. See the "Invoices in the merchant SDKs" row in the Merchant SDKs section. Unchanged and still open: no Cypress case follows a `hosted_invoice_url`; no PDF, e-mail or hosted invoice page; no taxes, discounts or credit notes; no subscriptions or recurring invoices; no dunning; `invoice.marked_uncollectible` and `invoice.payment_failed` are not emitted; `invoice.*` webhook bodies carry `lines.data` empty; no partial payments; and the dashboard has no invoice screen. **Updated 2026-09-10 (exp47, issue #91 items 3 and 4): two more of the recorded gaps are closed, and one of them only at the database.** (a) **What a refund does to a paid invoice is decided** (D5): the invoice stays `paid` and gains a nineteenth wire key, `amount_refunded` (migration `0042`), written by `Settlement::apply_refund_succeeded` in the same transaction that flips the `refunds` row `pending → succeeded`. It is a **gross** counter — not subtracted from `amount_paid`, not part of `amounts_add_up` — so `paid_means_nothing_remaining` needed **no amendment and did not get one**; the brief asked for that amendment and this deviates from it deliberately, because under the shape the issue decided a refunded paid invoice was already storable, and the design that would have made the amendment load-bearing (decrement `amount_paid`, push the difference into `amount_remaining`) makes a fully refunded invoice read `paid` with the whole bill *remaining* — the number `pay` mints an intent for. What is load-bearing is a sixth multi-column CHECK, `refunded_at_most_paid`, which makes the settlement fail closed; `amount_refunded_non_negative` is the seventh single-column one. **NONE OF IT IS REACHABLE**: no rail can refund (`mtn_momo::refund` is `NotImplemented`, Orange answers `Unsupported`), `POST /v1/refunds` is unrouted and `vpay_db::Refunds` still exposes no `create`, so `amount_refunded` is `0` in every deployment and no shipping binary calls that method — the four container-backed cases seed a `pending` refunds row with a raw `INSERT` exactly as `tests/refunds.rs` does. Two mutations were run: move the `amount_refunded` write out of the transaction → the over-refund case fails with the refund left `succeeded`; delete `refunded_at_most_paid` → the same case commits 5,500 refunded against a 5,000 bill. (b) **`POST /v1/invoices/{id}/pay` may omit both URLs** (D2) when the registration carries `merchant_clients[].invoices.success_url` / `.cancel_url`; a request that sends its own still wins, and a merchant with neither gets one `400` naming both. Validated at boot by `vpay_config`'s `validate_invoice_urls` **and again** at request time by the same `checked_forward_url` a passed URL goes through, with `every_invoice_url_shape_the_config_admits_is_admitted_by_the_request_path` holding the two crates' copies together. Mutation: ignore the configured default → the wire case fails. The suites are **15 → 16** container-backed cases in `invoices.rs` and **6 → 11** in `vpay-db`'s `repositories.rs`; drift **172 → 173** and the applied-migration count **40 → 41** (the branch said "39 → 40" and was right until issue #88's `0040` landed on master in PR #108 and the rebase brought it in). **Amended 2026-09-11 by the review**, which found the money key untested rather than mistested: `amount_refunded` was asserted at `0` everywhere, and `0` is also what a hard-coded literal in `InvoiceObject::render` and `vpay_sdk::Invoice`'s `#[serde(default)]` produce — both mutations passed every case in their crates. There are now a non-zero render assertion (`vpay-api`, **342 → 347** cases with the four `pay`-URL cases beside it), a non-zero decode plus a key-removed decode (`vpay-sdk`, **165 → 166**), and a `tokio::join!`ed pair of concurrent refund settlements, which is the only case that fails when the increment is a total read first inside the transaction. Still open and unchanged: **no Cypress case follows a `hosted_invoice_url`** and **the payer's checkout page still does not render the invoice number** — items 2 and 5 of issue #91, not attempted in this pass |
...
| Staff sign-in — `vpay_api::staff`, `vpay_api::staff_auth`, `vpay_db::{staff, staff_sessions, authorization_codes}` | ✅ | **New 2026-09-07 (exp24, [ADR-0017](adr/0017-staff-authentication.md)).** Seven unauthenticated routes under `/dash/v1` — `POST /staff/login`, `/staff/totp`, `/staff/password`, `GET /staff/session`, `POST /staff/logout`, `GET /oauth/authorize`, `POST /oauth/token` — **eight since 2026-09-10, the eighth being `GET /staff/session/stage`; see the amendment at the end of this row** — merged **beside** the protected reads rather than inside the bearer-token layer, because they exist to produce the credential that layer checks. Credentials: argon2id (19 MiB, t=2, p=1 — OWASP's first listed set) with a deployment pepper as argon2's secret input, and RFC 6238 TOTP (HMAC-SHA1, 30 s, ±1 step) whose secret is AES-256-GCM-sealed under a second deployment key. Sessions are server-side rows keyed by the **SHA-256** of an opaque 256-bit token, with an absolute 12 h bound and an idle 30 min bound, and signing out **deletes** the row — which is the server-side deny-list ADR-0009's Consequences left undecided. **Three security properties are compare-and-swaps in SQL rather than checks in Rust**, because in each the thing being prevented is a race: the TOTP replay guard (`record_totp_step`'s `last_totp_step < step`), the authorization code's single use (`consume_code`'s `consumed_at IS NULL`), and enrolment happening once (`enrol_totp`'s `totp_enrolled_at IS NULL`). **Every refusal on the sign-in path is one answer** — no such address, wrong password, disabled account, wrong code, replayed code, expired session, idle session, forged session, session at the wrong stage: one `401`, one sentence, the step in the log and never the body. The timing half is `verify_absent_account`, which costs a real argon2id verification for an address that has no account, so the two answers take the same time as well as the same shape. Rate limiting is in-process, fixed-window, per email **and** per IP, refused before any credential work, with both counters moved on every attempt including a refused one; the per-IP half was **dead as first delivered** and was fixed by the exp24 review (finding F2) — the peer address reaches a handler only through axum's `ConnectInfo`, neither `vpay-server` nor the test harness built its service with `into_make_service_with_connect_info`, so every attempt in the process shared one `ip:unknown` bucket and ten unauthenticated requests locked the whole deployment out of the dashboard for five minutes (`the_sign_in_rate_limit_is_per_source_address` drives two loopback source addresses through a real socket and is what would fail again). Behind a reverse proxy the peer is the proxy, so the per-IP budget bounds the deployment rather than the caller — no `X-Forwarded-For` is trusted, and ADR-0017's Consequences says why. Proof: 16 container-backed cases in `staff_sign_in.rs` (13 as delivered, plus the three the exp24 review added below) (the happy path; identical answers for a wrong password and an unknown address; a disabled account refused at once on **both** credentials a sign-in produces — the session and the already-minted bearer token — and a staff member **moved to another merchant** refused on the token they already hold, both of which the exp24 review had to add (findings F1 and F6: as first delivered `require_dashboard_token` never read the staff row at all, so everything it checked was a statement about the *token* and none of it was a statement about the *person*; a disabled staff member kept listing payment intents, and a reassigned one kept reading their old merchant's, for the rest of the 15-minute access-token TTL, while `staff_members.status` was documented as the per-person kill switch); a replayed TOTP code refused inside its own step while the next step's works; a PKCE verifier mismatch; a code exchanged twice; a code redeemed against another redirect URI, refused **and spent**; a staff member of another merchant refused a code at all; idle and absolute expiry; sign-out; a password-only session refused at `/authorize`; the printed password unable to reach `/dash/v1`; a deployment with no `staff_auth` serving no login; and the sign-in rate limit binding per **source address**), plus 26 unit cases in `vpay_api::staff_auth` pinned against RFC 6238 Appendix B, RFC 4648 §10 and NIST's own SHA-256 vector, and RFC 7636 Appendix B for the PKCE check. **What is not built:** no pages (see the row above), no sweep of expired sessions or codes, no `audit_log` (ADR-0008 wants one per dashboard *write* and none is mounted), no way to disable the dashboard *client* short of removing it from YAML, and no key rotation. **Read as a residual, not a claim:** `staff_sessions.access_token` holds a live bearer token for the length of its TTL, so a database dump yields usable `/dash/v1` tokens until they expire. Hashing it does not work — the dashboard's own server has to present it — and ADR-0017's Consequences says so plainly. **A second residual, left open on purpose:** signing out makes the token *unobtainable* (the row it is read from is gone) but not *invalid* — the JWT verifies until it expires, `signing_out_deletes_the_session_and_with_it_the_access_token` asserts exactly that, and closing it means binding every `/dash/v1` read to a live session row, which makes the surface stateful. **That is a maintainer decision the exp24 review surfaced rather than took.** **AMENDED 2026-09-10 (exp36, issue #79 items 1-3).** Three sentences in this row are withdrawn and one behaviour changed for every caller. (1) ~~Rate limiting is in-process~~ — it is **rows in `rate_limit_windows`** (migration `0038`), so every replica spends from one budget. The count is a single `INSERT … ON CONFLICT (id) DO UPDATE … RETURNING attempts`, which is what makes two replicas serialise on a row instead of both reading 9 and both writing 10; the key is the **SHA-256** of `<action>:<dimension>:<value>`, so an unauthenticated caller neither sizes a row nor gets an address with no account written down; and the same statement sweeps up to 32 elapsed rows `FOR UPDATE SKIP LOCKED`, sixteen times the two an attempt adds, which is what bounds a table whose keys a caller chooses. ADR-0017 refused a durable counter as "a denial-of-service amplifier of a different kind" — that objection is answered by those three properties rather than overruled, and its own Consequences had already named this "the first thing to revisit if a deployment runs many replicas". `two_replicas_share_one_sign_in_budget` boots two vpay servers over one Postgres and reads `429` on the sixth attempt against a configured budget of five, with one row carrying `attempts = 6`; the decisive mutation is giving each `SignInLimiter` instance its own counters again, which makes it read `[401 x 6]`. **The limits are configuration now** (`staff_auth.rate_limits`, one policy per action), defaulting to the ten-per-five-minutes this row shipped and to five per five minutes for the new current-password check. **Whether the shared default should now be BELOW ten is a maintainer decision this pass did not take** — ten was chosen for a limiter three replicas multiplied by three, and is strictly stricter now than it was; it is listed here as open rather than lowered in passing. (2) ~~no `X-Forwarded-For` is trusted~~ — `staff_auth.trusted_proxies` (addresses and CIDR blocks, **empty by default**, and empty is exactly what shipped) makes the client address the **first untrusted hop from the RIGHT** when and only when the transport peer is in the list. Taking the leftmost entry, which is the shape most "get the real IP" snippets have, is whatever the caller wrote and would have handed every caller a fresh bucket per request. Four fallbacks end at the peer: an untrusted peer, an unparseable hop, an all-trusted header, an absent header. It is **two** container-backed cases and not one, because either alone passes with the trusted-proxy check deleted: `a_forwarded_for_header_from_an_untrusted_peer_buys_no_fresh_budget` (six attempts, six claimed client addresses, one peer, `429`) and `a_forwarded_for_header_from_a_trusted_peer_is_the_client_address` (one claimed client exhausted, a second one's first attempt still served). An entry that does not parse takes the login down with a log line naming the index, rather than narrowing the list silently. (3) **`POST /staff/password` now requires the password in force and deletes every OTHER session of that staff member.** This is a **breaking change to that endpoint's contract** and to `frontends/apps/dashboard`'s form, which carries the field now. The argument for the current password being absent was written down in three places and was wrong for one reason: a session lives twelve hours and its two factors were presented once, at its start, so the credential protecting an irreversible takeover (the change also clears `password_change_required`) was **the session cookie alone**. The revocation half answers the question the endpoint had never answered — changing a password did nothing about the person you changed it because of, whose row stayed `authenticated` with a live `access_token` until the absolute bound. The caller's own session survives on purpose. `changing_a_password_needs_the_current_one_and_ends_every_other_session` is the guard, with two decisive mutations: deleting the `verify_password` makes the no-current-password case read `200`, and deleting the `delete_others` makes the other browser's next render read `200`. The dashboard vitest case `asks for the new password twice and for no current one` **asserted the old behaviour** and is replaced by one that asserts the field exists. **The suite is 16 -> 21 cases in `staff_sign_in.rs`** (measured 2026-09-10, 21 passed 0 ignored) -- **23 after the exp36 review added two** (`a_repeated_forwarded_for_is_one_chain_and_buys_no_fresh_budget` and `the_current_password_check_has_its_own_budget_and_it_is_the_sessions`), 23 passed 0 ignored, with three more in `vpay-db/tests/repositories.rs` and three unit cases in `client_address`. **Also 2026-09-10 (exp36, issue #88 item 2): a vpay the dashboard cannot reach is no longer a sign-out.** `requireStaff` sent a browser to `/signed-out` for **every** failure of `GET /dash/v1/staff/session`, and `server/api.ts` deliberately turns a rejected `fetch` into an `ApiFailure` with `status: 0` rather than throwing — so "vpay is restarting", "the connection was reset" and "vpay answered `503`" were indistinguishable from "your session is over", and a rolling restart signed every staff member out of the dashboard with no way to tell that from a real sign-out. `server/gate.ts::refusalFor` is the fix and is a pure function with five unit cases, because "a 503 signs everybody out" should be a red test and not something noticed during an incident: **`401` is the only status that ends a session**, which is exact rather than conservative because vpay answers `401` for every session refusal by design. Everything else — including a `403`, which on that route means something in front of vpay refused the app — renders the message and its request id on the page **with the cookie untouched**. `requireStaff` answers a `{ kind: 'outage' }` arm the three protected pages render through the new `ReadFailure` component, which also replaces two copies of the same inline `Alert`. The decisive mutation is widening `refusalFor` to `status >= 400`, which turns four of the five cases red. Dashboard vitest 151 -> 156. **Also 2026-09-10 (exp36, issue #88 item 4): the server actions' CSRF check compares a value no caller can set.** Next's own check (`app-render/action-handler`) compares `Origin` against `x-forwarded-host` when that header is present and against `Host` otherwise — **two caller-supplied values, which agree whenever the caller wants them to**. Behind a proxy that does not strip incoming forwarding headers (the default for several), `X-Forwarded-Host: evil.example` plus `Origin: https://evil.example` passes it, and an export of a `'use server'` file is a `POST` endpoint anything on the internet can reach while this app's session cookie is `SameSite=Lax` rather than `Strict`. `frontends/apps/dashboard/src/server/csrf.ts` compares `Origin` against `VPAY_DASHBOARD_PUBLIC_ORIGIN`, and `originIsAllowed`'s **signature** is the guard: it takes the `Origin` and the `Host` and has no parameter a forwarding header could arrive through, so the decisive mutation is a signature change — reproduced by preferring the caller-supplied host over the configured origin, exactly what Next does, which turns the forwarded-host case red. An absent `Origin` is refused; so is the literal `null` a sandboxed document sends. All four actions carry the two lines (there is no wrapper, because a wrapper around a `'use server'` export changes what Next registers), and `signOut` refuses by doing nothing at all rather than by clearing a cookie. **The variable is OPTIONAL and that is a compromise, recorded rather than glossed:** making it required would have refused every action on every deployment not yet reconfigured, including this repository's own compose stacks, which a concurrent change is editing — so with it unset the check falls back to the `Host` header, never `X-Forwarded-Host`, which is strictly stronger than Next's and **wrong behind a proxy that rewrites `Host`**, where every action is then refused with one sentence and a container-log line naming the variable. **The follow-up is to set it in `compose.demo.yml` and in `deploy/helm` and make it required**; this pass did not, and no compose file or helm value was touched. Dashboard vitest 156 -> 172. **What this pass did NOT do, from the same two issues:** the payer mask (#79 item 4 — it touches the `charges` write S5 owns), ~~the `/dash/v1` token re-mint before the 900 s TTL (#88 item 1 — the reactive re-mint `dash-read.ts` already does on a `401` stands, and a *proactive* one needs a `staff_sessions.access_token_expires_at` column, an API field and an e2e overlay with a short TTL)~~ **done 2026-09-10 by exp44 — see the last amendment on this row; the shopping list above turned out to be exactly right**, the pager's `has_more` Cypress case (#88 item 3) and the `@vpay/ui` select flake (#88 item 5). **`just test-e2e` was NOT run and that is not a skip:** the recipe's dashboard is on a hard-coded host port (`compose.demo.yml:301` publishes `127.0.0.1:3000:3000` with `!override`, the recipe polls `http://localhost:3000/` as a literal, and `compose.e2e.yml:263` registers the dashboard client's `redirect_uri` as `http://localhost:3000/dash/v1/callback`, which is matched byte for byte at `/authorize` and again at `/token`), the maintainer's own `vpay-demo` stack was holding 8080/3000/3001/3080/8082/8083 at the time, and parameterising that port is the change exp35 owns and is making concurrently. So **no browser has exercised the current-password field**: `dashboard.cy.ts`'s leg 4 is updated for it — a wrong current password refused in the browser, then the printed one — and it typechecks and lints, but it has not run. ~~Run `just test-e2e` once exp35's port work is in.~~ **Struck 2026-09-10 by the exp36 review: exp35's port work IS in (PR #94, merged 08:52), this branch was rebased onto it, and the recipe was run.** **REVIEWED 2026-09-10 (exp36 review). `just test-e2e` was run — at `demo_dashboard_port=13200`, on the rebased tree, PR #94 having landed — and it FAILED: `dashboard.cy.ts` 8 tests, 2 passing, 6 failing.** The first failure is leg 4, the current-password field this change exists for, and the cause is this branch's own: `POST /dash/v1/staff/password` answers `401` for a wrong current password now, and `src/server/actions.ts` still read ANY `401` from that endpoint as "the session is over" and cleared the cookie -- correct only while the endpoint had no credential to refuse. So a mistyped current password signed the staff member out instead of telling them, and the browser went to `/login` where the spec expected an alert (`(new url) http://localhost:13200/login`, `[role="alert"]` 0 elements); the other five failures are the cascade, because that spec is `testIsolation: false` and its retries replay an enrolment already committed. The `401` clears nothing now: a session that really is over is caught one render later by `PasswordPage`, which reads the session on every render and redirects. **Also 2026-09-10 (exp36 review, findings F4 and F5): two behaviours the change rests on were exercised by nothing.** (F4) `count_attempt` is one statement and it decides the window rollover, the reset value and the sweep INSIDE itself; the unit tests cover arithmetic over an integer the statement returns, the statement's own test asserts the TEXT of six fragments, and every case over a booted server runs inside one 300-second window -- so a `CASE` that never reset passed all of them, and its symptom in production is a staff member locked out of the dashboard for good, the durable lockout ADR-0017 refuses by name. Three cases in `vpay-db/tests/repositories.rs` run it now: with the reset arm dropped `an_elapsed_rate_limit_window_is_replaced_rather_than_extended` reads 5 where it demands 1, and with the sweep disabled `the_rate_limit_table_grows_by_one_window_and_is_then_swept` finds 1040 rows where it demands 60 or fewer. **The bound is also now stated as it is rather than as it reads:** "drains the table faster than they fill it" is true in the limit and not inside one window -- 1000 fresh keys inside one window leave 1000 rows, measured -- so the bound is two rows per attempt for the width of one window and then flat, roughly `2 x rate x 300` at steady state. (F5) The current-password budget (`change_password:session`) was exercised by nothing either: the case that proves the current password is required makes two wrong attempts against a default of five, so a `check_password_change` that had been deleted passed the suite, and the check it guards is an argon2id verification anybody holding a stolen cookie can drive at will. `the_current_password_check_has_its_own_budget_and_it_is_the_sessions` reads 401 where it demands 429 with that call deleted, and pins that a SUCCESSFUL change spends a unit too and that a second browser of the same person has its own budget. **Also 2026-09-10 (exp36 review, finding F7): vpay's origin check does NOT replace Next's -- both run, and an action needs both.** Measured against a booted stack by firing the real `signIn` action id with chosen headers: a foreign `Origin` whose `Host` AND `X-Forwarded-Host` agree with it is refused by vpay (which is item 4 proven live, and is exactly the request Next accepts), an absent `Origin` is refused, and with the variable unset the `X-Forwarded-Host` spoof is refused too. **But** with the variable set correctly and a proxy that rewrites `Host` and sends NO `X-Forwarded-Host`, vpay's check passes and **Next's own aborts the action with a 500** whose message names neither this app's sentence nor the variable. So setting `VPAY_DASHBOARD_PUBLIC_ORIGIN` is necessary and NOT sufficient behind such a proxy -- it must also send `X-Forwarded-Host`, which every ordinary reverse proxy does, and that deployment was measured working. The prose read as though this check had replaced Next's; `docs/flows/dashboard.md` carries the seven-row measured matrix now. Not coded around: the lever is `serverActions.allowedOrigins`, widening Next's own check is a security change wanting its own review, and no measured deployment needs it. **And the outage claim is corrected: a render with vpay stopped shows the message and keeps the cookie (`200`, no `Set-Cookie`, measured) but carries NO request id** -- `api.ts`'s `unreachable` sets `requestId: null` because there was no response. "The error page with the request id" is true of a vpay that answered `5xx` and not of a refused connection. **Also 2026-09-10 (exp36 review, finding F3): `VPAY_DASHBOARD_PUBLIC_ORIGIN` is SET now, where a browser can reach it.** The implementer recorded setting it as the follow-up and touched no compose file and no helm value, which left the configured comparison the one path nothing exercised. `compose.e2e.yml` now gives the dashboard `VPAY_DASHBOARD_PUBLIC_ORIGIN: http://localhost:${VPAY_DEMO_DASHBOARD_PORT:-3000}`, keyed to the same variable as the publication and `VPAY_DASHBOARD_REDIRECT_URI`, so `just test-e2e` drives every server action through the configured origin rather than through the `Host` fallback. The chart carries `dashboard.publicOrigin` (values, schema, README) and an eighteenth guard, `dashboard-public-origin`, which refuses a value that is not `scheme://host[:port]` -- a trailing slash or a bare hostname there refuses every server action on the dashboard, and checking the shape where the value is written is the only place that mistake is cheap. **No template reads it**, because this chart writes no dashboard workload, and the README says so beside the key. **It is still OPTIONAL, and that is now a decision rather than a deferral:** making it required belongs with the dashboard Deployment this chart does not yet write, because a required value on a workload nothing renders would fail a deployment for a setting nothing reads. **REVIEWED 2026-09-10 (exp36 review), and the allow-list had a second hole in it.** `client_address_from` read `HeaderMap::get` — the FIRST `X-Forwarded-For` field line only. A proxy may append its hop as a NEW field line rather than rewriting the caller's, and RFC 9110 s5.2 makes repeated field lines one comma-separated list in the order received; on such a deployment the entire chain being walked was the one the CALLER wrote, the "first untrusted hop" was whatever they put at the end of it, and the allow-list handed out a fresh rate-limit bucket per request rather than closing one -- the same hole the untrusted-peer case refuses, arriving from a trusted peer. It reads every line now, right to left across them, and an unreadable line ends the walk at the peer rather than being stepped over. `a_repeated_forwarded_for_is_one_chain_and_buys_no_fresh_budget` is the case: two field lines over a real socket, and with `get` restored it reads `[401 x 6]` against a budget of five. See [plans/exp36-staff-followups-notes/opus-review.md](plans/exp36-staff-followups-notes/opus-review.md). See [plans/exp36-staff-followups-notes/opus.md](plans/exp36-staff-followups-notes/opus.md). **AMENDED 2026-09-10 (exp44, issue #88 and the exp36 review's finding F6, which that review recorded and did not fix): a mistyped six-digit code no longer signs a staff member out.** `POST /dash/v1/staff/totp` answers one `401` for a **wrong code** and for every session it refuses — this row's own "every refusal is one answer" — and `frontends/apps/dashboard/src/server/actions.ts` read all of them as "the session is over", clearing the session cookie *and* the sealed enrolment blob. That is the identical shape as F1 one route over, and it was worse on a first sign-in: the retry then carried no secret to commit, so the enrolment could not be completed at all. F6 could not be closed by deleting the two lines, and that is why it stayed open: `/login/totp` read **no** session, only the cookie's presence, so with the cookie kept a session that really was over left somebody typing codes at a form that could never accept one. The page reads one now, and `GET /dash/v1/staff/session` cannot serve it — that route is refused for a session that has not presented a code, with the same `401` a dead session gets. So there is an **eighth** unauthenticated staff route, `GET /dash/v1/staff/session/stage`, which answers `{"stage": "pending_totp" | "authenticated"}` through `load_session` and **publishes nothing about the person**: a caller at that stage has presented a password and no second factor, so a display name, an email or a merchant id would be identity moved to the wrong side of it. It deliberately does not touch `last_seen_at` — being asked for a credential is not use of a session. The decision itself is `server/gate.ts::totpGateFor`, a pure function with five vitest cases, for `refusalFor`'s reason: a `401` from the **stage read** ends the session, an unreachable vpay renders the failure and keeps the cookie (issue #88 item 2, on this route), and an already-`authenticated` session goes on to the payments list rather than being asked to replay a code. Proof: `a_wrong_code_leaves_the_session_live_and_the_stage_route_says_so` and `a_disabled_account_is_refused_at_the_stage_route` over a booted server (**`staff_sign_in.rs` 23 -> 25 cases**), and a new leg in `dashboard.cy.ts` that types a wrong code in a real browser and asserts the alert, the surviving cookie **and the surviving QR** — the enrolment panel is the assertion that would be missed. Decisive mutations, all three run: `session_stage` calling `authenticated_session` instead of `load_session` (`a_disabled_account_is_refused_at_the_stage_route` reads `401` where it demands `200`); `totpGateFor` mapping any failure to `'dead'` (the outage case signs everybody out mid-sign-in); `submitTotp` clearing the cookie on a `401` again (the browser leg lands on `/login`). **What this admits, stated rather than glossed:** somebody holding a stolen `pending_totp` session may keep guessing codes at one screen instead of re-presenting a password per attempt. It is bounded by the same limiter, which counts a wrong code against the shared sign-in budget. **ALSO 2026-09-10 (exp44, issue #88 item 1): the `/dash/v1` token is replaced BEFORE it expires, and its TTL is a deployment setting.** This row's "what this pass did NOT do" listed it; it is done. The token lives 900 s and a session up to twelve hours, and `requireStaff` ran the authorization-code leg only when the row carried **no** token — so a quarter of an hour into every sign-in every render was an error box (the exp28 review's finding F4), and the reactive retry that review added to `dash-read.ts` fixed the symptom at the cost of a refused request in front of every read. Three pieces: **migration `0040`** adds `staff_sessions.access_token_expires_at`, written in the same statement as the token and paired with it by `staff_sessions_token_expiry_is_paired` (multi-column, therefore invisible to `cratestack migrate baseline`, so `a_session_token_without_its_expiry_is_refused_by_the_database` asserts it against a real Postgres and the drift constraint list gains an eighteenth entry — drift itself is unchanged at 172/24/19, because the column matches its model); `GET /staff/session` carries `access_token_expires_at` and `access_token_ttl_seconds`, and the app does **not** read the JWT's own `exp` because it holds the token and does not verify it; and `server/gate.ts` gains `REMINT_AFTER_FRACTION = 0.8` and a `stale-token` arm. **There is still no refresh token and there is not going to be one** — the refresh *is* the authorization-code leg, which re-reads the staff row, the active status, the merchant binding and `password_change_required` on every mint, so a re-mint is more checking than carrying one token rather than a way around any. `stale-token` is a separate arm from `needs-token` because there is something to fall back on: a vpay that cannot be **reached** for the re-mint leaves the page rendering with the token in hand, which has not expired. A `401` is not fallen back on, stale or not — that is `/authorize` refusing this session on this request, and the exp24 review's finding F1 is what reading on would repeat. **`staff_auth.access_token_ttl_seconds`** (900 by default, `garde`-bounded 10..=3600) is configurable where `/v1`'s constant deliberately is not: that constant's argument is about a number *merchants* build against, and nothing outside this deployment ever receives a dashboard token. What it buys is the case nothing could otherwise exercise — `demo_staff_token_ttl` sets **thirty seconds** on the demo/e2e stack (a deliberate difference from production, recorded in `docs/runbooks/demo.md`), so `dashboard.cy.ts` crosses the margin and the expiry in one leg in a real browser. Proof: `the_dashboard_token_is_re_minted_from_a_live_session_and_from_nothing_else` over a booted server — `expires_in` and the recorded expiry both the configured 10 s and not 900; a live session minting a **different** token and moving the expiry; and the leg refused for a **disabled** staff member, one **moved to another merchant**, an **idle** session and a **signed-out** one, each with the same session restored afterwards as a control so the refusal is attributable to the mutation. **`staff_sign_in.rs` 25 -> 26 cases**, `postgres_smoke.rs` +1, dashboard vitest 177 -> 182. Decisive mutations, all run: writing `crate::op::ACCESS_TOKEN_TTL_SECS` back into the dashboard OP config (`expires_in` reads `900` against a configured `10`); `staleTokenFor` answering `false` for an absent or unparseable expiry; and dropping the margin — for which the browser leg asserts that `access_token_expires_at` **moved while the old expiry had not yet passed**, because with `dash-read.ts` still retrying, a page renders identically whether the token was replaced early or after a `401`. **What migration 0040 does to existing rows, stated rather than buried:** it **clears every `access_token` already stored** instead of inventing an expiry for one, because there is no honest value to write. Nobody is signed out — a session with no token is the state every session is in between the second factor and its first render, and the next render mints one. **Not done in this pass:** no browser leg disables a staff member mid-session (there is no cy.task that can, and `the_dashboard_token_is_re_minted_...` case 2 plus `disabling_a_staff_member_refuses_their_live_session` prove it over a booted server instead); `deploy/helm` does not carry the new setting, so a chart deployment gets the 900 default. **Gate, on the final head `a7e6971`:** `just ci` **exit 0** read from a file — `fmt-check`, `clippy -D warnings`, the twelve `verify-*` gates (`verify-migrations` 40 files, `verify-links` 1034 links in 197 files), `test-rust` **1665 run, 1665 passed, 0 skipped** across 45 binaries against a real Postgres, `test-doc` **111 passed, 1 ignored**, `verify-ignored` **0 ignored (expected 0), 45 binaries, 1665 total**, `lint-web`, `test-web` (dashboard **182 in 21 files**), `deny` all four ok. **And `just test-e2e` WAS run**, on its own compose project at `demo_dashboard_port=13800`: **19 of 19 Cypress tests passing** (dashboard 9, checkout 1, shop-hosted 3, shop-embedded 6), exit 0. See [plans/exp44-session-remint-notes/opus.md](plans/exp44-session-remint-notes/opus.md). **Reviewed 2026-09-10 ([plans/exp44-session-remint-notes/opus-review.md](plans/exp44-session-remint-notes/opus-review.md)), and it was NOT safe as delivered** — not because anything it built is wrong, but because **two of the security properties it states most emphatically were enforced by nothing**. `staff_auth.access_token_ttl_seconds`' `10..=3600` bound was claimed in four places and asserted in none: measured, `#[garde(range(...))]` replaced with `#[garde(skip)]` left `-p vpay-config` at **112 passed, 0 failed**. And "`session_stage` does not touch `last_seen_at`" — the property that stops the new unauthenticated route being a way to hold a half-authenticated session open past its thirty-minute idle bound — was stated in four places and measured by nothing: one `touch` line added to the handler left `staff_sign_in` + `boot_coherence` + `dashboard_read_surface` at **43 of 43 green**. Both now have a test that fails against its mutation. Also fixed: `gen-demo-keys`' new TTL freshness check was a `grep -F` on a line ending in a bare number, so `demo_staff_token_ttl=3` against an overlay written for `30` read as fresh and served ten times the requested TTL (measured against the real overlay); `docs/reference/vpay-api.md` still said "the seven staff routes" in its second mention; and `dash-read.ts` still attributed the dashboard token's 900 s to `vpay_api::op::ACCESS_TOKEN_TTL_SECS`. **No way through the new surface was found**: the stage route publishes one word and no identity, is refused for every session `load_session` refuses, unlocks nothing (every other staff route calls `authenticated_session`), and is not a session-id oracle — tokens are 256 bits of `OsRng` stored as their SHA-256. **Gate on the review head `HEAD`:** `fmt-check`, `clippy -D warnings` and the twelve `verify-*` gates green (`verify-migrations` 40 files, `verify-links` 1042 links in 204 files); `test-doc` **111 passed, 1 ignored**; `verify-ignored` **0 ignored (expected 0), 46 binaries (expected 46), 1674 total**; `lint-web` clean; `test-web` all green (dashboard **182 in 21 files**); `deny` all four ok; and `just test-e2e` on its own compose project (`demo_project=exp44-review`, `demo_dashboard_port=14000`) **19 of 19 passing**, exit 0. **`test-rust` did not complete on the review head, and the reason is this host and not this branch**: six consecutive runs aborted at five *different* tests, every one of them on testcontainers' `failed to create a container: Timeout error`, with `dockerd-rootless` logging `libnetwork: restoring thread network namespace failed: operation not permitted` — the rootless-Docker fault this machine has had before — while a single `docker run` succeeded throughout. The best complete run is **1671 of 1671, 0 skipped, exit 0** on the rebased head *before* the review's three added tests; the furthest the review head reached is **1522 of 1523 run**, the one failure being that container timeout. Each of the three added tests was run individually against a real Postgres and each fails against its own mutation. **This row must not be read as claiming a green `just ci` on the review head.** |
...
| Worker job loop (`vpay_worker::run_loop`, `vpay-worker-bin`) | 🟡 | **New 2026-09-03 (Step 4); the row that used to say "there is no job loop".** `run_loop(pool, adapters, rails, policy, concurrency, grace, worker_id, shutdown)` runs `--worker-concurrency` (default 4) claim→run→settle tasks, sleeps 1 s on an empty claim, and is the **same function** the integration suite drives — no `#[cfg(test)]` variant, no injected clock, no second implementation (AGENTS.md rule 1). At boot, **before** seeding, it reaps leases older than `RecoveryPolicy::lease`: a worker that was `SIGKILL`ed leaves rows nothing can claim, and if the dead worker held `sweep:expired` the only other reaper is itself among them (`a_lease_stranded_by_a_crash_is_freed_at_boot_before_any_sweep_runs`). It then reaps again every `lease / 2`, floored at the 1 s idle poll so a short lease cannot become a hot loop (`a_lease_that_expires_while_the_worker_runs_is_reaped_on_its_own_timer`). It seeds `sweep:expired` and `scan:live` in one transaction, because a partial seed is a deployment whose backstop is silently absent. **Observability is one log line a minute**, at INFO, message `job loop gauge`: `worker_id` (hostname/pid/random — the same value in `locked_by`, so an operator can tell which pod holds a stuck job), cumulative `claimed`/`finished`/`rescheduled`/`dead_lettered`/`lost`, and `queue_behind_seconds` = `now() - min(run_at)` over runnable unleased rows (`null` for an empty queue, parked rows excluded). `lost` counts jobs whose lease was reaped while this worker was running them; **any non-zero value is a handler outrunning the lease**, and it is counted separately precisely so it cannot hide inside its neighbours. **🟡, not ✅, for three reasons.** (1) **No metrics endpoint and no dashboard** — the gauge line and the log pipeline are the whole of it, and no alerting rule anywhere consumes `alert = true`. (2) **The loop has never run outside a test or a developer's `just demo`**; there is no CI e2e job for it and no deployment. (3) The contradiction path it logs is untested — see the Settlement row. **`--worker-concurrency` has an upper bound as well as a lower one since 2026-09-10 (issue #63)**: above `vpay_db::MAX_CONNECTIONS / 2` — 5 — `vpay-server worker` refuses to boot (exit 78, `StartupError::WorkerConcurrencyExceedsPoolSize`), before it opens the pool, because a webhook fan-out re-running after a crash holds two pooled connections at once. The ceiling is the conservative reading of a measurement, not a cliff: `the_boot_guards_maximum_concurrency_fits_the_pool_and_a_saturated_one_starves_the_reaper` shows five simultaneous fan-outs fitting with the lease reaper served in 1.2 ms, and the pool starving only when all ten connections are pinned — and `fan_out_events` is a singleton job, so one process never has more than one fan-out in flight anyway. More throughput is more replicas; see [flows/crash-safety.md](flows/crash-safety.md#worker-concurrency-and-the-pool). Unit coverage is 36 tests in `vpay-worker`; the behavioural coverage is 20 `worker_recovery` + 3 `worker_e2e` integration tests, all container-backed and all passing on the tree this note describes |
| Real signal crash tests (`backends/tests/integration/tests/worker_kill9.rs`) | ✅ (two of three `SIGKILL` points, plus both arms of the `SIGTERM` drain) | **Updated 2026-09-10 (issue #85, exp38): a third scenario, and about the other signal.** `a_worker_sigtermed_mid_delivery_drains_it_and_the_merchant_is_told_exactly_once` settles an ordinary payment, lets one of **two** running workers claim the `payment_intent.succeeded` delivery, and sends that worker a real `SIGTERM` while its POST is inside the merchant's WireMock receiver (a third container, the same mapping tree `compose.e2e.yml` mounts). It exits **0** with `graceful shutdown complete, exiting` and **no** `Drain::TimedOut` warning, having *finished the delivery*: one `webhook_deliveries` row `succeeded` with `attempt = 0`, its job deleted, and **one** signed POST in the receiver's own journal, accepted by `vpay_sdk::webhooks::verify` under the configured secret — plus the same four-record exactly-once invariant the two kill cases assert. **Determinism is by construction, not by margin:** the receiver's 6 s delay sits strictly between `vpay_worker::webhooks::WEBHOOK_REQUEST_TIMEOUT` (10 s) and `--shutdown-grace-seconds` (20 s), and both comparisons are `const` assertions in the test file, so lowering either shipping budget fails the **build**; and the case reads the signalled worker's own transcript and fails unless `shutdown signalled; draining in-flight jobs` precedes `webhook delivered`, so a signal that landed too late reports that instead of passing. **Two decisive mutations, both measured rather than argued:** removing the drain (`Drain::Clean` returned the instant the signal is seen) fails with *the worker never logged `webhook delivered`* — the send cut off after the receiver had already accepted it; and removing `locked_at IS NULL` from `vpay_db::Jobs::claim` fails on the double-send assertion, `left: 2  right: 1`, with two byte-identical signed POSTs at the receiver. The second is why the second worker **co-runs** rather than restarting: a worker spawned after the signal has booted only after the drain is over, so the lease it must not steal no longer exists and the property would be proved vacuously. **What it does not cover:** `Drain::TimedOut` under a real signal (that path is `worker_e2e.rs`'s `a_drain_that_runs_out_of_grace_releases_every_lease_it_still_holds`, in-process), and **no case restarts a worker after a graceful stop** — the half of the hand measurement that is still only a measurement. `the_sigterm_scenarios_confirm_with_an_msisdn_that_arms_no_rail_mapping` is a container-free guard keeping the rail boring for that case. **Extended the same day by that change's sabotage review, which closed both of those gaps with one case and added a second guard.** `a_drain_that_runs_out_of_grace_under_a_real_signal_exits_1_and_hands_the_lease_back` is the same staging with `--shutdown-grace-seconds 2` against the same 6 s receiver — a third `const` assertion keeps the *factor of two*, not a mere `<` — and **both** workers signalled at once, the way stopping a Deployment does it. The victim exits **1** (its own `std::process::exit(1)`, asserted not to be a death by signal), with no `graceful shutdown complete` and no `webhook delivered`; its warning carries `released=1`, the shipping code's own count of the leases it handed back; the `deliver_webhook` job is back in the queue **unleased** with `attempts = 1`; and a **restarted** worker then finishes it — the half of the hand measurement no case had staged, and which after a *clean* drain could only be staged vacuously. It also writes down what nothing had: **a timed-out drain is at-least-once at the receiver.** The merchant gets two byte-identical signed POSTs of one event, both verifying under the endpoint secret and both carrying the same `Vpay-Event-Id`, while `webhook_deliveries.attempt` is still `0` — an aborted task records no failure, so the only trace of the first attempt is `jobs.attempts`, and the case asserts both numbers. The money is untouched, on the same four records. Two more mutations, both measured: skip `release_all` on the timed-out path → `released=0`; make that arm exit `0` → `left: Some(0)  right: Some(1)`. Ten consecutive runs, 10 passed, 17.8–23.6 s. The review's second guard, `the_slow_receiver_mapping_is_the_delay_both_sigterm_cases_are_built_on`, pins `slow-ack.json`'s delay and path to the constants they are transcribed into and refuses any other receiver mapping that could match that path at an equal or better priority: `RECEIVER_ACK_DELAY` was pinned to the two shipping budgets and to nothing else, so an `11000` in the mapping cost a 20-second container run to discover and read as a drain regression. It now fails in 6 ms. **New 2026-09-04 (Step 8, lane D).** Real `Child::kill()` (`SIGKILL`) to the shipping `vpay-worker-bin` mid-status-query and to the shipping `vpay-server` mid-`requesttopay`, against real Postgres + WireMock, no test double. `a_worker_killed_mid_poll_settles_the_charge_exactly_once_after_its_lease_is_reaped`: the lease and an unanswered `provider_requests` row are the only trace after the kill (the exit status is asserted *signalled with 9*, so a process that chose to `exit(1)` could not stand in for one that was killed); a second worker's boot-time reap recovers it and the charge settles exactly once — one `charges` row, one event, `amount_received` once, and exactly one submit plus two status queries in the rail's own journal. `a_server_killed_mid_submit_leaves_a_charge_the_worker_settles_without_a_second_submit`: the server dies with the POST issued and unanswered; the worker recovers by polling, never resubmitting (one submit in the journal). Guard-failure proof: disabling `vpay_db::jobs::reap_expired_leases` leaves the second worker booted, healthy and permanently unable to claim the stranded job — the test fails deterministically inside its bound. **Two clocks are simulated and nothing else is:** `age_the_dead_workers_lease` moves `jobs.locked_at` ten minutes back (guarded on the dead worker's own `worker_id`) because `RecoveryPolicy`'s five-minute lease has no CLI override, and — **added on the gate branch, after lane G** — `age_the_crashed_charge` moves `charges.created_at` ten minutes back (guarded on `state = 'submitting'`) because lane G's minimum charge age makes a freshly-killed server's charge indistinguishable from a live confirm. That second clock is load-bearing and was measured to be: without it the server-kill case failed on the gate (TRY 3 FAIL, 63 s) with the worker correctly waiting. **Kill point 1** (before any `provider_requests` row exists) is not staged against a real process — there is no network call to interrupt at that moment — and remains proven only by `worker_recovery.rs` writing the state directly. **Neither Orange Money nor a real rail is exercised here**: both cases use `mtn_momo`, and the rail is a WireMock container, so the claim is "the recovery table is executed correctly under a real signal", not "the rails behave as documented" |
...
- ~~**Content is a design sketch, not full coverage.** It models only entities
  ...
  `Refund`. What survives the correction is the *rule* the bullet was stating,
...
had to *rename* `providers.flow`'s hand-named CHECK after the fact because
...
and asserts *which* one comes back **green**, because
