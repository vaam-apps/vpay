...
| Dashboard app | 🟡 | Renders a scaffold notice and a design-system smoke test. **No data, no auth, no routes** — and that is **unchanged as of 2026-09-06 (exp23)**, which is worth saying because the *backend* moved that day and this did not. Slice 1's brief asked for sign-in, a payments list, a payment detail and sign-out; none was built. The reason is upstream of the frontend: a dashboard session needs a token, and no grant this deployment serves can issue one (see the "`/dash/v1` read surface" and "Dashboard auth" rows under Backend). Pages behind a session that cannot exist could not be reached, screenshotted or driven by Cypress, so building them would have produced UI nobody has seen work. `dashboard.cy.ts` still asserts the scaffold notice, unchanged, and `frontends/apps/dashboard` still has **zero tests**. **Unchanged again on 2026-09-07 (exp24), and the reason above is now WITHDRAWN.** A session can exist: [ADR-0017](adr/0017-staff-authentication.md) built the staff table, the credentials, the sessions and the authorization-code grant, and `staff_sign_in.rs` signs a person in end to end. So the pages were no longer blocked and were still not built — the honest statement of why is **scope**, not a blocker: the backend slice was as much as one pass delivered, and the brief it was written to said to deliver the grant end to end first and report the pages as not done rather than stubbing them. `frontends/apps/dashboard` is byte-identical to the base commit, `dashboard.cy.ts` is unchanged, `just test-e2e` was not run, and there are no screenshots because there is nothing to screenshot |
