...
**Also missing, and larger:** nothing writes or reads a `refunds` row at all.
There is no refunds repository in `vpay-db` (`vpay_db::RefundRow` is a row
shape with no queries and says so), no `/v1/refunds` route in
`vpay_api::V1_ROUTES`, and no writer for `charge.refunded` /
`charge.refund.updated` — both types are in the `type_is_a_documented_event`
vocabulary and neither has ever been emitted. So `RefundObject` has never
crossed a wire; what the tests above prove is that the *contract* holds, not
that a refund works.
