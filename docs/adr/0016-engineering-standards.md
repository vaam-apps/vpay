...
| `Transfer`                  | `backends/crates/vpay-adapter-mtn-momo/src/wire.rs`     | Models MTN's camelCase **Disbursements** wire (`externalId`) — `RequestToPay`'s reason, on the refund body. `Payee` beside it needs no row because it renames every field. Added 2026-09-15 (RFC-0003 § 5).                                                                                       |
