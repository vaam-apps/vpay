...
only — losing an empty table costs nothing _yet_, and that changes the day
something writes to it, not the day this ADR is revised.

_This ADR's own status is still **Proposed** and unreleased on this branch,
...
| `provider_requests`                     | `0016`, `0020`                 | One row per _attempt_ to call a rail: operation, attempt number, `status_code` (with `NULL` = no answer and `0` = answered without an HTTP status), `error_kind`, `sent_at`/`responded_at`                                                                                                                                                                                                                                                                                                                                                                    | Yes                                                                                                                                                                                                                                                                                                                                                                                                                                                   | The only diagnosis path there is. [ADR-0004](0004-musl-mimalloc.md) put no shell in the runtime image precisely because "diagnosis happens through logs, traces and the `provider_requests` audit trail". Losing it deletes step 2 of [runbooks/unresolved-charges.md](../runbooks/unresolved-charges.md): whether the rail was ever asked, and whether it ever answered, becomes unanswerable                                                                                                                                                                                                              |
| `oauth_client_assertion_jtis`           | `0011`                         | Spent `private_key_jwt` assertion ids, with `expires_at`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Yes                                                                                                                                                                                                                                                                                                                                                                                                                                                   | **A replay window re-opens.** The primary key _is_ the single-use guard (RFC 7523 §3 point 7). Restoring to an instant before an assertion was recorded makes every captured assertion whose `exp` has not yet passed spendable again, and nothing anywhere reports that it happened                                                                                                                                                                                                                                                                                                                        |
...
per-transaction balance invariant is _aggregate_ and no row-level `CHECK` can
...
drill — but it says nothing about whether that row has already been _run_.
...
are consequences a well-executed restore _causes_ rather than prevents: the
