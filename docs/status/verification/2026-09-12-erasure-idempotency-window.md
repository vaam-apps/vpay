...
**Rebased onto `origin/master` on 2026-09-13, and two of these numbers moved.**
[PR #142](https://github.com/vaam-apps/vpay/pull/142) (issue #113) is the only
commit the rebase brought in that touches `backends/`, and it adds one `--lib`
case to `vpay-db` and one to `vpay-api`: on the rebased head
`cargo nextest run -p vpay-db` prints **188 passed** and
`cargo nextest run -p vpay-api` **363 passed**. The numbers above are what those
commands printed when they were run, and stay as recorded. Re-measured unchanged
on the rebased head: both doctest rows, and the `customers` suite at 24.
...
because those files were tracked and modified rather than new. **Also moved by
the 2026-09-13 rebase:** the rebased head counts **1519 links in 331 tracked
files**, the fifteen extra files being those
[PR #142](https://github.com/vaam-apps/vpay/pull/142),
[PR #139](https://github.com/vaam-apps/vpay/pull/139),
[PR #141](https://github.com/vaam-apps/vpay/pull/141) and
[PR #156](https://github.com/vaam-apps/vpay/pull/156) brought with them. 1 487 is
what this commit measured and is left as measured.
...
**Rebased onto `origin/master` on 2026-09-13, and three of these numbers
moved.** [PR #142](https://github.com/vaam-apps/vpay/pull/142) (issue #113)
landed underneath this branch and added one `--lib` case to each crate, so
`cargo test -p vpay-db --lib` prints **78 passed** and
`cargo test -p vpay-api --lib` **363 passed** on the rebased head, and
`verify-links` **1519 links in 331 files**. The rows above are what the
commands printed when they were run, and stay as recorded.
