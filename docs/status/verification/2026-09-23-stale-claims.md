...
| #   | Where                                                                                                                    | Said                                                                                                                              | Evidence on `master`                                                                                                                                                                                                                          | Wrong since                                                                                            |
| --- | ------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| 1   | `backends/apps/vpay-server/src/main.rs` module doc                                                                       | the API server "never calls a payment rail — that is the worker's job"                                                            | `vpay_api::v1::payment_intents::submit_to_rail` awaits `adapter.submit` on confirm; `v1::refunds` calls `adapter.refund`; `v1::account_holders` calls `adapter.account_holder_name`                                                           | Step 3, 2026-09-03 (the sentence is from the 2026-08-09 scaffold)                                      |
| 2   | [payment-lifecycle.md](../../flows/payment-lifecycle.md) state diagram                                                   | `requires_action --> processing` and `requires_action --> failed : submit response lost`                                          | `vpay_core::next_status` gives `requires_action` no edge; `SETTLEABLE_STATUSES` settles or fails straight from it; a lost redirect submit leaves the intent in `requires_payment_method`                                                      | Step 4, 2026-09-03 (the edges are from the scaffold)                                                   |
| 3   | [account-holder-lookup.md](../../flows/account-holder-lookup.md)                                                         | "MTN's real sandbox has never been called — for this or for any other operation"                                                  | [2026-09-15.md](2026-09-15.md): token, `requesttopay` and status query ran against the real sandbox; `basicuserinfo` did not, so the lookup half still stands                                                                                 | 2026-09-15                                                                                             |
| 4   | [failures.md](../../flows/failures.md)                                                                                   | "neither rail's real sandbox has ever been called"                                                                                | the same run, which produced no decline                                                                                                                                                                                                       | 2026-09-15                                                                                             |
| 5   | [backend.md](../backend.md) adapter matrix                                                                               | MTN real sandbox "⛔ never called"; wire calls "never against MTN or Orange"                                                      | the same run                                                                                                                                                                                                                                  | 2026-09-15                                                                                             |
| 6   | [reconciler.md](../../flows/reconciler.md)                                                                               | callback sketch ends `→ 200 OK`; Status lists "No fan-out"                                                                        | `vpay_api::provider_callback` answers `202` (unknown reference too), `404` for no adapter, `400` for a bad body; `vpay_worker::webhooks` fans out `pending` events                                                                            | `202`: from the route's first day, Step 8, 2026-09-04. Fan-out: Step 5, 2026-09-03                     |
| 7   | [errors.md](../../flows/errors.md)                                                                                       | "no job loop exists to call `JobError::decision()`"                                                                               | `vpay_worker::run_loop` calls `error.decision(..)` for every job whose handler returns `Err`                                                                                                                                                  | Step 4, 2026-09-03                                                                                     |
| 8   | [docs/api/README.md](../../api/README.md)                                                                                | event `type` is "one of the thirteen", eight written; invoice has "eighteen keys"; `verify-status` prints zero, "no `501` at all" | CHECK in migration `0039` lists fifteen types, [webhooks.md](../../flows/webhooks.md) names thirteen writers; `the_invoice_object_is_the_documented_nineteen_keys`; `an_unbuilt_rail_refund_fails_and_releases_its_reservation` asserts `501` | types: 2026-09-10. keys: 2026-09-10 (migration `0042`). `501`: from the day it was written, 2026-09-16 |
| 9   | [merchant-auth.md](../../flows/merchant-auth.md), [resource-contract.md](../../flows/merchant-auth/resource-contract.md) | `"expires_in": 300`; no scheduled idempotency or `jti` sweep; `sdks/nodejs` "has still never spoken to a vpay"                    | `ACCESS_TOKEN_TTL_SECS = 900`; `vpay_worker::handlers::sweep_expired` deletes both hourly; `invoices.live.test.ts` and `refunds.live.test.ts`, run by CI's `e2e` job                                                                          | `300`: from the day it was written, 2026-09-02. Sweeps: Step 4, 2026-09-03. Node: 2026-09-10           |
| 10  | [README.md](../../../README.md)                                                                                          | `just demo` brings up "eight" services, the dashboard "stays down … no login"; layout calls the dashboard "a scaffold"            | the justfile's `demo_services` names nine, including `dashboard`; [dashboard-sign-in.md](../../runbooks/demo/dashboard-sign-in.md)                                                                                                            | exp28, 2026-09-07                                                                                      |
| 11  | [examples/README.md](../../../examples/README.md)                                                                        | refund creation "has no route"; both examples make a balance and a refund call                                                    | `POST /v1/refunds` mounted 2026-09-16; neither example calls either route                                                                                                                                                                     | 2026-09-16                                                                                             |
| 12  | [dashboard-auth.md](../../flows/dashboard-auth.md) Status                                                                | "The rate limit is per replica"                                                                                                   | `rate_limit_windows` (migration `0038`); `two_replicas_share_one_sign_in_budget`                                                                                                                                                              | 2026-09-10                                                                                             |
| 13  | [sdks/stripe-js/README.md](../../../sdks/stripe-js/README.md)                                                            | "Not yet on the registry … no workflow publishes"; "Step 5c ships push-only … return trip not wired"                              | `publish-stripe-js-sdk` in `release.yml` (2026-09-18); `npm view` lists `0.2.1` (2026-09-19) to `0.4.1`; [browser-checkout.md](../../flows/browser-checkout.md) § "The redirect gap (D4) — closed 2026-09-04"                                 | registry: 2026-09-19. Return trip: 2026-09-04                                                          |
| 14  | [frontends/apps/dashboard/README.md](../../../frontends/apps/dashboard/README.md)                                        | "Webhooks, checkout sessions … are not built"; the BFF's handlers are called by nothing                                           | deliveries screen `f9c7d2c` (2026-09-13), checkouts screen `8be7850` (2026-09-14); `app/(dash)/layout.tsx` wires `dashDataProvider("/api/dash")`                                                                                              | deliveries: 2026-09-13. Checkouts: 2026-09-14. BFF: 2026-09-12                                         |
