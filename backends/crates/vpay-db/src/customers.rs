...
///
/// One of migration `0034`'s three new `type_is_a_documented_event` allows,
...
/// [`Customers::create`] sets it to `created_at`.
...
pub trait Customers: Send + Sync {
    /// Inserts a customer and returns the row the database actually stored —
    /// including `seq` and `updated_at`, which it filled in itself — so a
    /// caller never has to re-read to render its response.
    ///
    /// # Why this is one hand-written statement and not `create(..)`
    ///
    /// `metadata` is `JSONB NOT NULL` with no `DEFAULT` and is not declared
    /// on `model Customer`, so `CreateCustomerInput` has no field for it and
    /// the generated `INSERT` would omit the column entirely — a `23502`, by
    /// construction, which is exactly what migration `0034` left the
    /// `DEFAULT` off to guarantee.
    ...
    /// # Errors
    ///
    /// [`DbError::UniqueViolation`] naming the primary key if `id` is
    /// already taken — which cannot happen for a freshly minted `cus_…`.
    /// [`DbError::Query`] for anything else, including
    /// `at_least_one_identifier`, `name_length`, `email_length`,
    /// `phone_is_a_canonical_msisdn` and `metadata_is_object`, every one of
    /// which the API refuses first with a `400` naming the parameter — so
    /// reaching one here is a vpay bug rather than a merchant's mistake.
    async fn create(&self, new: &NewCustomer) -> Result<CustomerRow, DbError>;
    ...
    /// indistinguishable.
    ///
    /// Hand-written for [`Customers::create`]'s reason inverted: the row has
    ...
    /// Applies a patch and returns the row as it now stands, or `None` if
    /// this merchant has no such customer.
    ///
    /// # Why the merchant filter is in the statement and not a preceding read
    ///
    /// A read-then-write would let a handler compare tenants in Rust, which
    /// is the mistake this module exists to make inexpressible. The `UPDATE`
    /// carries `WHERE merchant_id = $1 AND id = $2`, so a foreign customer
    /// updates zero rows and `None` is the same answer a missing id gives.
    ///
    /// `last_used_at` moves with the write: an update *is* a use (migration
    /// `0034`'s definition), so a merchant who edits a customer every month
    /// never has it swept.
    ///
    /// An empty patch is refused **by the caller**, not here:
    /// [`CustomerPatch::is_empty`] exists so `vpay-api` can answer a bodiless
    /// `POST` with the object unchanged. Reaching this method with one would
    /// write `updated_at` and `last_used_at` and nothing else, which is
    /// harmless and is not what the wire contract says happens.
    ///
    /// # Errors
    ///
    /// [`DbError::Query`], including the same five CHECKs
    /// [`Customers::create`] lists.
    async fn update(
        &self,
        merchant_id: &str,
        id: &str,
    ...
    /// (`data_is_object`) or a `type` outside migration `0034`'s eleven, both
...
mod tests {
    ...
    /// keeps [`super::Customers::create`] a hand-written statement.
    ///
    ...
    async fn a_generated_customer_insert_cannot_carry_metadata() {
        ...
        assert!(
            ...
            "`metadata` became expressible through the generated input. Before moving \
             `Customers::create` onto it, re-read `model Customer`'s GAP note: \
