//! The `events` repository (`backends/migrations/0018_create-events.sql`) —
//! the durable log behind webhook fan-out and `GET /v1/events`.
//!
//! [`crate::TxRepositories::insert_in_tx`] takes a connection and has no pooled
//! variant: an event committed separately from the transition it describes is
//! either a webhook for something that did not happen or a transition no
//! merchant is ever told about. [`Events::pending_page`] is the drain's
//! backlog query; [`Events::list_page`] and [`Events::get_by_id`] are the
//! merchant's own merchant-scoped read, the documented fallback for a webhook
//! they missed.
//!
//! Events are written for terminal transitions only — `docs/status.md` is the
//! record of which types are live, and `docs/reference/vpay-db.md`
//! §"`events`" says why the write is where it is.

use sqlx::PgConnection;
use time::OffsetDateTime;

use crate::error::{DbError, classify_write};
// `GET /v1/events` pages exactly as `GET /v1/payment_intents` does, so it
// takes that module's cursor type rather than declaring a second one — see
// [`Events::list_page`] for why a divergence here would be a merchant's bug, not a
// cosmetic inconsistency.
use crate::payment_intents::ListPage;

/// The `evt_…` id minter, re-exported from `vpay_core::ids`.
///
/// Re-exported rather than left to each caller to find, because the id has
/// to be generated by the *caller* — [`crate::TxRepositories::insert_in_tx`] and the settlement
/// transaction both take one as an argument, so a retried settlement can
/// reuse the id it already logged instead of minting a second name for one
/// event — and a caller reaching for "some unique string" is how a table
/// whose ids merchants dedupe on (`docs/flows/webhooks.md`) acquires a row
/// with a UUID in it. The vocabulary stays closed in `vpay-core`; this is
/// only a signpost to it.
pub use vpay_core::ids::event_id;

/// Every column of `events`, in one place so the two queries cannot drift on
/// what [`EventRow`] decodes.
const COLUMNS: &str = "id, seq, merchant_id, livemode, type AS event_type, object_id, data, \
                       fanout_state, created_at";

/// One `events` row, exactly as stored.
#[derive(Debug, Clone, PartialEq, sqlx::FromRow)]
pub struct EventRow {
    /// The `evt_…` id merchants dedupe deliveries on.
    pub id: String,
    /// Fan-out order (migration 0018). Database-generated.
    pub seq: i64,
    /// Who the event belongs to. Copied from the object at emit time, not
    /// joined — see [`NewEvent::merchant_id`].
    pub merchant_id: String,
    /// Live or test money, copied from the object for the same reason.
    pub livemode: bool,
    /// One of the seven types `type_is_a_documented_event` allows. Named
    /// `event_type` because `type` is a Rust keyword; the column is `type`,
    /// which `COLUMNS` aliases rather than renaming the field with a
    /// `#[sqlx(rename)]` — the alias keeps the two queries and this struct
    /// agreeing in one place instead of two.
    pub event_type: String,
    /// The object this event is about (`pi_…`, `ch_…`, `re_…`).
    pub object_id: String,
    /// The wire object as it was when the event happened — a snapshot, so a
    /// delivery that succeeds an hour later still describes the transition
    /// it is reporting rather than whatever is true by then.
    pub data: serde_json::Value,
    /// `pending` until fan-out completes, then `done` — or `failed` when the
    /// drain has abandoned it (migration 0024; see [`Events::record_fanout_failure`]).
    /// `vpay_worker::webhooks` owns both writers.
    ///
    /// `fanout_attempts` is deliberately **not** on this struct: no reader of
    /// an event branches on it, and the one writer that cares gets the new
    /// value back from its own `RETURNING`. Selecting a column nothing uses
    /// is how a row struct starts mirroring the table instead of its callers.
    pub fanout_state: String,
    /// When the event was emitted.
    pub created_at: OffsetDateTime,
}

/// The columns a caller supplies: [`EventRow`] minus the three the database
/// owns (`seq`, `created_at`) or defaults (`fanout_state`, always `pending`
/// at insert — an event that is born `done` is an event nobody delivers).
#[derive(Debug, Clone, PartialEq)]
pub struct NewEvent {
    /// `evt_…`, from [`event_id`], generated by the caller.
    pub id: String,
    /// The merchant this event will be delivered to. Denormalised onto the
    /// row rather than reached by joining the object, because fan-out reads
    /// the backlog by `seq` alone and must not have to know which of three
    /// tables `object_id` points into to work out where to send it.
    pub merchant_id: String,
    /// From the object, never from configuration read at delivery time: a
    /// deployment's `livemode` can change between emit and delivery, and the
    /// event describes what was true when it happened.
    pub livemode: bool,
    /// One of the seven documented types. The database refuses the rest.
    pub event_type: String,
    /// The id of the object the event is about.
    pub object_id: String,
    /// The wire object, which must be a JSON object (`data_is_object`).
    pub data: serde_json::Value,
}

/// Appends one event **inside the caller's transaction**.
///
/// See the module comment for why there is no pooled variant.
///
/// # Errors
///
/// [`DbError::UniqueViolation`] on `events_pkey` if the `id` has already
/// been emitted — which for a retried settlement is the *correct* refusal:
/// two rows would be two deliveries of one transition. [`DbError::Query`]
/// otherwise, including a `type` outside the documented seven or a `data`
/// that is not a JSON object, both of which are vpay bugs.
pub(crate) async fn insert_in_tx(
    tx: &mut PgConnection,
    new: &NewEvent,
) -> Result<EventRow, DbError> {
    let sql = format!(
        "INSERT INTO events (id, merchant_id, livemode, type, object_id, data) \
         VALUES ($1, $2, $3, $4, $5, $6) \
         RETURNING {COLUMNS}"
    );

    sqlx::query_as::<_, EventRow>(&sql)
        .bind(&new.id)
        .bind(&new.merchant_id)
        .bind(new.livemode)
        .bind(&new.event_type)
        .bind(&new.object_id)
        .bind(&new.data)
        .fetch_one(&mut *tx)
        .await
        .map_err(classify_write)
}

/// What one recorded fan-out failure left behind: the new attempt count, and
/// the state the event is now in.
#[derive(Debug, Clone, PartialEq, Eq, sqlx::FromRow)]
pub struct FanoutFailure {
    /// `events.fanout_attempts` **after** the increment.
    pub attempts: i32,
    /// `events.fanout_state` after it: `pending` if attempts remain,
    /// `failed` if this failure was the one that spent them.
    ///
    /// Carried as a `String` for the reason every other closed vocabulary in
    /// this crate is: the vocabulary belongs above the persistence layer, and
    /// the database (`fanout_state_is_known`) is what actually closes it.
    pub state: String,
}

#[async_trait::async_trait]
pub trait Events: Send + Sync {
    /// The oldest undelivered events, in emission order, at most `limit` of
    /// them.
    ///
    /// Ordered by `seq` and not by `created_at`: two events emitted inside the
    /// same microsecond tie on a timestamp, and a fan-out cursor over a
    /// non-total order either repeats a delivery or — the one that matters —
    /// skips one entirely. `events_pending_idx` is exactly this query's
    /// predicate and order, so the scan is over the *backlog* rather than over
    /// the log.
    ///
    /// Not merchant-scoped, unlike every read in [`crate::payment_intents`]:
    /// this is the worker's queue, not an API surface, and a per-merchant sweep
    /// would deliver in merchant order rather than in the order things happened.
    /// `GET /v1/events` is a different query, and it will be merchant-scoped.
    ///
    /// `fanout_state = 'pending'` also excludes `'failed'` (migration 0024) —
    /// which is the whole point of that state existing. An event the drain has
    /// abandoned after [`Events::record_fanout_failure`]'s ceiling would otherwise sit at
    /// the head of every page forever, holding one of the caller's `limit` slots
    /// and re-raising its alert on every pass.
    ///
    /// # Errors
    ///
    /// Returns [`DbError::Query`] if the read fails.
    async fn pending_page(&self, limit: i64) -> Result<Vec<EventRow>, DbError>;

    /// Counts one failed fan-out pass against an event, abandoning it once
    /// `max_attempts` of them have failed.
    ///
    /// # Why this is a separate statement and not part of the caller's
    /// transaction
    ///
    /// It is the *failure* of that transaction that it counts, and that
    /// transaction has rolled back. A counter incremented inside it would roll
    /// back with it, so the event would be retried forever at zero attempts,
    /// which is exactly the state this column exists to end. One statement on
    /// the pool is its own transaction, and it is the whole of the write: the
    /// increment and the terminal flip are one `UPDATE`, so two workers racing on
    /// the same event cannot both observe the transition (the `fanout_state =
    /// 'pending'` guard admits one of them) and the alert is therefore raised
    /// once.
    ///
    /// `max_attempts` is the caller's — `vpay_worker::webhooks::FANOUT_MAX_ATTEMPTS`
    /// — rather than a constant here, so the ceiling lives with the handler that
    /// spends it and there is one spelling of it.
    ///
    /// `Ok(None)` means the event was not `pending`: another drain fanned it out
    /// while this one was failing on it, or it has already been abandoned. There
    /// is nothing to count and — importantly — nothing to alert about.
    ///
    /// # Errors
    ///
    /// Returns [`DbError::Query`] if the write fails.
    async fn record_fanout_failure(
        &self,
        event_id: &str,
        max_attempts: i32,
    ) -> Result<Option<FanoutFailure>, DbError>;

    /// One merchant's page of events, newest first — `GET /v1/events`.
    ///
    /// The cursor semantics are [`crate::PaymentIntents::list_page`]'s,
    /// deliberately to the letter, and [`ListPage`] is that module's type rather
    /// than a second copy of it: two list endpoints whose paging disagrees about
    /// what `starting_after` means, or about whether `has_more` looks forwards
    /// or backwards, is a difference a merchant discovers by writing a loop that
    /// silently skips objects. See that function's doc comment for the full
    /// argument — in particular why an unknown, deleted or *foreign* cursor id
    /// yields an empty page rather than falling back to the newest rows, and why
    /// both cursor subqueries are merchant-scoped.
    ///
    /// `events_merchant_seq_idx` (migration 0018) is exactly this query's scope
    /// and order.
    ///
    /// Merchant-scoped in SQL, unlike [`Events::pending_page`]: that one is the worker's
    /// queue over every merchant's backlog, and this is an API surface where the
    /// scope is the authorisation. There is no unscoped variant, and no
    /// `merchant_id` defaulting — an event belongs to exactly one merchant, so a
    /// read that could omit the scope is a read that could leak one merchant's
    /// payment history to another.
    ///
    /// # Errors
    ///
    /// Returns [`DbError::Query`] if the read fails.
    async fn list_page(
        &self,
        merchant_id: &str,
        page: &ListPage,
    ) -> Result<(Vec<EventRow>, bool), DbError>;

    /// Reads one event by id, with **no merchant scope** — the webhook
    /// deliverer's read, and nothing else's.
    ///
    /// # Why this exists beside [`Events::get_by_id`], which is scoped
    ///
    /// `vpay_worker::webhooks::handle_deliver` reaches an event from a
    /// `webhook_deliveries` row it got from a job it just claimed. It has no
    /// merchant to scope by: the merchant is a *property of the event*, and it is
    /// the value the deliverer needs in order to resolve the endpoint's secrets
    /// from configuration. Passing a merchant id in would mean inventing one, and
    /// the only place to invent it from is this row.
    ///
    /// **Not reachable from any `/v1` handler, and it must stay that way.** The
    /// scope on [`Events::get_by_id`] is the authorisation — an unscoped read on a
    /// merchant surface is one merchant's payment history served to another. The
    /// same distinction [`Events::pending_page`] makes against [`Events::list_page`]: a worker's
    /// queue is not an API surface.
    ///
    /// # Errors
    ///
    /// Returns [`DbError::Query`] if the read fails.
    async fn get_unscoped(&self, id: &str) -> Result<Option<EventRow>, DbError>;

    /// Reads one event *for this merchant* — `GET /v1/events/{id}`.
    ///
    /// `None` means "no such event for you", covering both a missing id and
    /// another merchant's id. The two are one answer for the same reason
    /// [`crate::PaymentIntents::get_for_merchant`] makes them one: a `404` that
    /// could be told apart from a `403` lets anyone enumerate which `evt_…` ids
    /// exist across the whole deployment.
    ///
    /// # Errors
    ///
    /// Returns [`DbError::Query`] if the read fails.
    async fn get_by_id(&self, merchant_id: &str, id: &str) -> Result<Option<EventRow>, DbError>;
}

#[async_trait::async_trait]
impl Events for crate::repository::PgRepositories {
    async fn pending_page(&self, limit: i64) -> Result<Vec<EventRow>, DbError> {
        // Postgres refuses a negative LIMIT, and a zero-row page is never what a
        // caller means.
        let limit = limit.max(1);
        let sql = format!(
            "SELECT {COLUMNS} FROM events WHERE fanout_state = 'pending' ORDER BY seq LIMIT $1"
        );

        sqlx::query_as::<_, EventRow>(&sql)
            .bind(limit)
            .fetch_all(&self.pool)
            .await
            .map_err(DbError::Query)
    }

    async fn record_fanout_failure(
        &self,
        event_id: &str,
        max_attempts: i32,
    ) -> Result<Option<FanoutFailure>, DbError> {
        sqlx::query_as::<_, FanoutFailure>(
            "UPDATE events \
         SET fanout_attempts = fanout_attempts + 1, \
             fanout_state = CASE WHEN fanout_attempts + 1 >= $2 THEN 'failed' ELSE 'pending' END \
         WHERE id = $1 AND fanout_state = 'pending' \
         RETURNING fanout_attempts AS attempts, fanout_state AS state",
        )
        .bind(event_id)
        .bind(max_attempts)
        .fetch_optional(&self.pool)
        .await
        .map_err(classify_write)
    }

    async fn list_page(
        &self,
        merchant_id: &str,
        page: &ListPage,
    ) -> Result<(Vec<EventRow>, bool), DbError> {
        // Postgres rejects a negative LIMIT outright, and a zero-row page is
        // never what a caller means. `vpay-api` owns the real ceiling.
        let limit = page.limit.max(1);
        let backwards = page.ending_before.is_some();
        let direction = if backwards { "ASC" } else { "DESC" };

        let sql = format!(
            "SELECT {COLUMNS} FROM events \
         WHERE merchant_id = $1 \
           AND ($2::TEXT IS NULL \
                OR seq < (SELECT seq FROM events WHERE id = $2 AND merchant_id = $1)) \
           AND ($3::TEXT IS NULL \
                OR seq > (SELECT seq FROM events WHERE id = $3 AND merchant_id = $1)) \
         ORDER BY seq {direction} \
         LIMIT $4"
        );

        let mut rows = sqlx::query_as::<_, EventRow>(&sql)
            .bind(merchant_id)
            .bind(page.starting_after.as_deref())
            .bind(page.ending_before.as_deref())
            .bind(limit.saturating_add(1))
            .fetch_all(&self.pool)
            .await
            .map_err(DbError::Query)?;

        let has_more = i64::try_from(rows.len()).unwrap_or(i64::MAX) > limit;
        if has_more {
            rows.truncate(usize::try_from(limit).unwrap_or(usize::MAX));
        }
        if backwards {
            rows.reverse();
        }

        Ok((rows, has_more))
    }

    async fn get_unscoped(&self, id: &str) -> Result<Option<EventRow>, DbError> {
        let sql = format!("SELECT {COLUMNS} FROM events WHERE id = $1");

        sqlx::query_as::<_, EventRow>(&sql)
            .bind(id)
            .fetch_optional(&self.pool)
            .await
            .map_err(DbError::Query)
    }

    async fn get_by_id(&self, merchant_id: &str, id: &str) -> Result<Option<EventRow>, DbError> {
        let sql = format!("SELECT {COLUMNS} FROM events WHERE merchant_id = $1 AND id = $2");

        sqlx::query_as::<_, EventRow>(&sql)
            .bind(merchant_id)
            .bind(id)
            .fetch_optional(&self.pool)
            .await
            .map_err(DbError::Query)
    }
}
