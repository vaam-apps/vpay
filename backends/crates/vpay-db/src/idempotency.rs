...
//!
//! [`claim`] issues `INSERT ... ON CONFLICT DO UPDATE ... RETURNING` and
...
//! fires only for an *expired* row; see [`claim`]'s own docs.
//!
...
//! key cannot get stuck: [`store`] completes it (the response is now
//! replayable), [`release`] hands it back (the outcome is one a retry should
//! re-execute rather than replay), and [`sweep_expired`] — or the reclaim in
//! [`claim`] — collects it 24 hours later if the process that held it died
...
//! `state = 'in_flight'` alone, R1's [`release`] would delete R2's live
...
//!
//! So [`claim`] returns the `claim_id` the database minted for *that* claim
//! ([`IdempotencyClaim::Fresh`]), and [`store`] and [`release`] both carry
//! `AND claim_id = $n`. A caller holding a superseded id therefore matches
//! no row, and [`store`] says so with
...
pub enum IdempotencyClaim {
    /// This process owns the key: do the work, then call [`store`] or
    /// [`release`] with the `claim_id` handed back here.
    Fresh {
        /// Which claim this is, so a later [`store`] or [`release`] cannot
...
/// rows [`sweep_expired`] would have deleted and for no others. Without it,
/// a key whose owning request died before it could [`store`] or
/// [`release`] anything would be answered [`IdempotencyClaim::InFlight`]
...
/// possible if [`sweep_expired`] deleted a 24-hour-old row, or if the
/// request that owned it called [`release`], in that window — is reported as
...
/// What [`store`] did with the response it was given.
///
...
///
/// The counterpart to [`store`], for every outcome that must **not** become
...
/// that was never claimed — and the next [`claim`] then takes it through the
...
/// correctness does not depend on the sweep, because [`claim`] reclaims an
