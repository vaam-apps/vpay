...
/// What [`SigningKeys::ensure_active_signing_key`] found, and therefore whether it wrote.
///
...
/// [`SigningKeys::rotate_signing_key`] and [`SigningKeys::ensure_active_signing_key`] cannot drift on
/// it. Retire first, then insert — [`SigningKeys::rotate_signing_key`]'s doc comment
...
/// *currently active* is not this error: [`SigningKeys::ensure_active_signing_key`]
/// answers [`ActivationOutcome::AlreadyActive`] before it ever gets here,
/// and [`SigningKeys::rotate_signing_key`] asked to re-insert the live key is a caller
...
pub trait SigningKeys: Send + Sync {
    ...
    async fn publishable_signing_keys(&self) -> Result<Vec<SigningKey>, DbError>;
    ...
    ///
    /// Returns [`DbError::Query`] if the read fails.
    async fn active_signing_key_kid(&self) -> Result<Option<String>, DbError>;
    ...
    /// so it queues behind any concurrent [`SigningKeys::ensure_active_signing_key`] rather
    ...
    /// in [`SigningKeys::rotate_signing_key`]: this layer never invents a window.
    ///
...
#[async_trait::async_trait]
impl SigningKeys for crate::repository::PgRepositories {
    async fn publishable_signing_keys(&self) -> Result<Vec<SigningKey>, DbError> {
    ...
    async fn ensure_active_signing_key(
        ...
        let previous = sqlx::query_scalar::<_, String>(
            "SELECT kid FROM oauth_signing_keys WHERE active LIMIT 1",
        )
        .fetch_optional(&mut *tx)
        .await
        .map_err(DbError::Query)?;
