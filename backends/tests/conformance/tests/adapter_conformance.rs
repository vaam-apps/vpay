...
/// Whether the rail under test advertises refunds — and, when it does not,
/// the assertion that it says so permanently.
///
/// [`claims_account_holder_lookup`]'s shape, for the same reason: the cases
/// below must not be silently skipped on a rail that has no refund API, they
/// must *assert* the absence. A rail flipped to `supports_refunds: true`
/// tomorrow starts running every one of them without an edit here.
...
/// answers `Unsupported`.
///
/// _(Until 2026-09-15 this case's job was to keep `mtn_momo::refund`'s
/// `NotImplemented` token honest while Disbursements was unbuilt. The token
/// is retired and the call is written, so what is left here is the weaker,
/// still-useful half: whatever a refunding rail answers, it is not "this rail
...
/// which is a wire case against a container.)_
#[rstest]
#[case::mtn_momo(RailUnderTest::MtnMomo)]
#[case::orange_money(RailUnderTest::OrangeMoney)]
#[tokio::test]
